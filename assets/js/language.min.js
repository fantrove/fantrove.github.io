 ...
 * ปรับปรุงเพิ่มเติม: เพิ่มการคาดคะเน/จัดการ "lsvg" (predicted svg)
 * - รองรับ marker ใหม่ @lsvg@ และ @lsvg:ID@ ใน worker (เหมือน @svg@ แต่เป็น marker แบบ "preferred front placement")
 * - หากการแปลไม่ระบุการจัดวาง SVG ใด ๆ (ไม่มี @svg@/@lsvg@ และไม่มี <svg> ในเนื้อหาแปล) แ��[...]
 *   ระบบจะคาดคะเนและแทรก marker แบบ internal (lsvg) ที่ตำแหน่งเริ่มต้นของเนื้อหา (front-most) —[...]
 * - หากมีการระบุ @lsvg@ ในสตริงแปล จะยึดตำแหน่งนั้น (แต่ยังคงความพยายามในการ [...]
 * - ระบบ reconciliation ยังคงพยายาม preserve identity ของ <svg> (ไม่ replace element ถ้า match) เพื่อรักษา event listeners / state
 *
 * พฤติกรรมโดยย่อ:
 * - ถ้ามีข้อมูลการวาง svg ตัวใดตัวหนึ่งในสตริงแปล (เช่น @svg@, @svg:id@, @lsvg@ หรือ inline <svg>), ร[...]
 * - ถ้าไม่มีข้อมูลใด ๆ แต่ DOM มี <svg> อยู่แล้ว ระบบจะ "คาดคะเน" ว่า svg ตัวแรก (หรือ�[...]
 *
 * รักษา logic เดิมอื่น ๆ ทั้งหมด (worker pool, cache, mutation observer, selector UI, slot marker ฯลฯ)
 */
...
`;
  ...
  _replaceDOMWithMarkerReplace(el, parts) {
    ...
    normalized.forEach(p => {
      if (p.type === 'text') {
        ...
        if (!htmlStr) return;
        // If html contains an <svg> try parse as SVG first
        ...
        // explicit lsvg in translation -> treat like svg marker but mark as lsvg
    ...
    if (!containsExplicitSvgOrLsvg && existingSvgsAll.length > 0) {
      // insert predicted lsvg marker at front if not already present
    ...
    // New: anchors tracking for reuse to preserve href/attributes/eventlisteners
    const existingAnchorsAll = Array.from(el.querySelectorAll ? el.querySelectorAll('a') : []).slice();
    const usedAnchors = new Set();

    // Helper: find svg by id or single available
    const resolveSvgMarkerGlobal = (id) => {
      ...
      // if id not given or not found, use first available unused svg
      const available = existingSvgs.filter(s => !usedSvgs.has(s));
      if (available.length >= 1) {
        // pick first available (this will be the one moved/reused)
    ...
    // Helper: find slot element by name
    const resolveSlotMarkerGlobal = (name) => {
      ...
      // if no name provided and only one slot exists, use it
    ...
    // Helper: find anchor element to reuse (preserve href/attrs/event listeners)
    ...
    for (let i = 0; i < newNodes.length; i++) {
      ...
      // Handle svg marker (including predicted lsvg markers)
      if (newNode && newNode.__svgMarker) {
        const svgRef = resolveSvgMarkerGlobal(newNode.id);
        if (svgRef) {
          // If this newNode was created as predicted (unshifted) we ensure it's placed front-most:
          if (newNode.__predicted) {
            // Move svgRef to very first child of el
            try {
              if (el.firstChild !== svgRef) {
                el.insertBefore(svgRef, el.firstChild);
              }
              // rebuild existing array because we moved nodes
              const idxOld = existing.indexOf(svgRef);
              if (idxOld !== -1) {
                existing.splice(idxOld, 1);
                existing.splice(0, 0, svgRef);
              }
              // adjust readIndex to account for front insertion if we are not already at start
              if (readIndex === 0) {
                // consumed the front-most, advance
                readIndex = 1;
              } else {
                // leave readIndex as-is, we will continue reconciliation
              }
            } catch (e) {}
            continue;
          }

          // otherwise, normal svg marker: move svgRef to current position if needed
          ...
          // not found -> create empty svg element (id if provided)
      ...
      // Handle anchor reuse/preservation: if the translation produced an <a> element, try to reuse existing anchor
      if (newNode && newNode.nodeType === 1 && newNode.tagName && newNode.tagName.toLowerCase() === 'a') {
        ...
        if (anchorRef) {
          ...
          // Update anchor text if translation provided text (preserve href/attrs/eventlisteners by reusing anchor)
          try {
            ...
            // If translation supplied attributes (rare with @a marker), copy them over (will override existing attrs)
          ...
          // no existing anchor to reuse -> insert new anchor (imported, so attributes/text preserved)
      ...
      if (currentOld) {
        ...
        // If currentOld is SVG but newNode isn't and no svg marker provided -> preserve svg by moving readIndex forward
        ...
        // If currentOld is a preserved slot element but newNode isn't a slot -> keep it
  ...
  _createMarkerNode(marker) {
    if (marker.type === 'text') {
      ...
      // If translation provides inner text, set it. We intentionally do NOT set href here
      // so that reconciliation logic can reuse existing anchor elements (preserving href/attributes/event listeners).
