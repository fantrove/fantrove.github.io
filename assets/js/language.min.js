...
class LanguageManager {
  ...
  async loadLanguageData(languageCode) {
    ...
    for (let attempt = 1; attempt <= this.RETRY_ATTEMPTS; attempt++) {
      try {
        const response = await fetch(`/assets/json/lang/${languageCode}.min.json`);
        if (!response.ok) throw new Error(`HTTP error! status: ${response.status}`);
   ...
   *
   * อัปเดตเพิ่ม: รองรับการแปลข้อความทั้งหมด (รวมถึงข้อความปกติและข้อความในลิงค์)
   * โดยไม่ไปยุ่งกับองค์ประกอบ img หรือ svg ให้ยังคงอยู่ในตำแหน่งเดิม
   *
   * รองรับ marker สำหรับลิงค์ในรูปแบบ @a[ข้อความที่ต้องการ]@ 
   * (ถ้าเป็น "@a@" ระบบจะไม่แปลข้อความภายในแท็ก <a> และคงไว้ข้อความเดิม)
   *
   * รองรับ marker สำหรับการแทรก <br> โดยระบุเป็น @br ซึ่งจะถูกแทนที่ด้วยแท็ก <br>
   *
   * รองรับ marker สำหรับ <strong> โดยระบุเป็น @strong ข้อความ@ ซึ่งจะถูกแทนที่ด้วย 
   * <strong>ข้อความ</strong>
   *
   * รองรับการแปล DOM หลายๆ element พร้อมกันโดยใช้ Promise.all
   */
  async translatePageContent(languageData) {
    const elements = document.querySelectorAll('[data-translate]');
    // Process all translation tasks concurrently
    await Promise.all(
      Array.from(elements).map(async (el) => {
        const key = el.getAttribute('data-translate');
        if (!languageData[key]) return;
        let translation = languageData[key];
        // แทนที่ marker @br ด้วยแท็ก <br>
        translation = translation.replace(/@br/g, "<br>");
        // แทนที่ marker @strong ด้วยแท็ก <strong> โดยแทนที่ข้อความที่อยู่ภายใน
        translation = translation.replace(/@strong(.*?)@/g, "<strong>$1</strong>");
        // ตรวจสอบว่ามีองค์ประกอบ img หรือ svg อยู่ใน element หรือไม่
        const hasMedia = el.querySelector('img, svg');
        // ตรวจสอบ marker สำหรับลิงค์ (@a...@)
        if (/@a(.*?)@/.test(translation)) {
          if (!hasMedia) {
            // หากไม่มี img/svg ให้สร้าง innerHTML ใหม่ตาม marker สำหรับลิงค์
            ...
            const newHTML = parts.map((part) => {
              ...
              if (markerMatch) {
                ...
                // กรณีที่ marker เป็น "@a@" (ไม่มีข้อความ) ให้คงข้อความเดิมของ <a>
            ...
            // หากมี img/svg ให้ปรับเฉพาะข้อความในแท็ก <a> โดยไม่ยุ่งกับ img/svg
            const regex = /@a(.*?)@/g;
            let matches = [];
            let match;
            ...
            anchorElements.forEach((a, index) => {
              // กรณี marker เป็น "@a@" ให้ข้ามการแปลข้อความในแท็กนั้น
              if (matches[index] && matches[index].trim() !== "") {
                a.textContent = matches[index];
              }
            });
            // นอกเหนือจากแท็กลิงค์ ให้แทนที่ข้อความใน text nodes โดยไม่ยุ่งกับ img หรือ svg
  ...
  /**
   * แทนที่ข้อความใน text nodes เท่านั้น (ไม่กระทบกับองค์ประกอบ HTML อื่น ๆ เช่น img, svg)
   */
  replaceTextOnly(element, newText) {
    element.childNodes.forEach((node) => {
      // หาก node เป็น text node ให้แทนที่ข้อความ
      if (node.nodeType === Node.TEXT_NODE && node.textContent.trim()) {
        node.textContent = newText;
      }
      // หาก node เป็น element แต่ไม่ใช่ img หรือ svg ให้เรียกใช้งานแบบ recursive
