 ...
 * @version 2.4.0
 *
 * เวอร์ชั่นนี้มีการปรับปรุงประสิทธิภาพให้รวดเร็วและทันเวลากว่าการอัพเดทที่มาจาก browser ด้วยเทคนิคขั้นสูง:
 * - ลด UPDATE_DELAY ลงเป็น 20 ms เพื่อลดเวลา wait
 * - ใช้ requestIdleCallback อย่างเต็มที่เพื่อให้ระบบทำงานในเวลาที่เบราว์เซอร์ว่าง
 * - ลดการเข้าถึง DOM ซ้ำซ้อนและใช้ batch update ผ่าน DocumentFragment
 * - ใช้ microtask scheduling (Promise.resolve().then(...)) ในบางส่วนที่ต้องการลำดับความเร็วสูง
 * - ปรับปรุงการใช้งาน MutationObserver ให้ throttle event processing
 *
 * การทำงานทั้งหมดและการเก็บข้อมูล (IndexedDB สำหรับ DSL และ JSON config) ยังคงไม่เปลี่ยนแปลง
 * โดย <html> จะถูกจัดการให้ attribute lang ถูกเซ็ตโดยระบบของเราเท่านั้น
 */
class LanguageManager {
  constructor() {
    ...
    this.UPDATE_DELAY = 20;  // ลด delay เพื่อให้ตอบสนองได้เร็วขึ้น
  
    // Throttling for MutationObserver
  ...
  async initialize() {
    ...
    try {
      ...
      // กำหนดเริ่มต้น <html> ให้ lang เป็นค่าว่าง เพื่อให้ระบบจัดการเอง
   ...
   *
   * ระบบจะจัดการ attribute lang ของ <html> ให้เป็นภาษาที่เลือกโดยระบบ
   */
  async updatePageLanguage(language, options = { updateHistory: true }) {
    ...
    try {
      ...
      // รักษา <html> lang ให้เป็นค่าว่างก่อน จากนั้น update โดยระบบของเรา
      document.documentElement.setAttribute("lang", "");
      ...
      // ใช้ microtask scheduling เพื่อลดเวลา delay
  ...
  /**
   * สังเกตการเปลี่ยนแปลงของ DOM ด้วยการ throttle ด้วย timeout
   */
  observeMutations() {
