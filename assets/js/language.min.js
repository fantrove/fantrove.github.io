 ...
 * @version 2.3.1
 *
 * เวอร์ชั่นนี้จะบล็อคการระบุภาษาของ browser ใน HTML อัตโนมัติ โดยลบ attribute "lang"
 * ออกจาก <html> ตอนเริ่มต้น เพื่อให้ระบบของเราสามารถอัปเดทภาษาได้อย่างรวดเร็ว โดยที่การอัปเดท
 * ภาษาจะเกิดขึ้นเฉพาะเมื่อระบบของเราทำการเปลี่ยนภาษาเอง
 */
class LanguageManager {
   ...
   * เริ่มต้นระบบ
   *
   * เพิ่มการลบ attribute "lang" จาก <html> เพื่อบล็อค browser จากการระบุภาษาอัตโนมัติ
   */
  async initialize() {
    if (this.isInitialized) return;
    // ลบ attribute "lang" เพื่อให้ระบบของเราเป็นผู้จัดการ
    ...
    for (let attempt = 1; attempt <= this.RETRY_ATTEMPTS; attempt++) {
      try {
        ...
        if (await this.validateConfig(newConfig)) {
          ...
          // บันทึก config ลง IndexedDB แทน LocalStorage
  ...
  async updatePageLanguage(language, options = { updateHistory: true }) {
    ...
    try {
      ...
      // ระบบของเราจะเป็นผู้อัปเดทภาษาที่ <html> แทน browser
