 ...
 * - <body style="opacity:0"> (ใส่เองใน HTML) จะถูก set opacity=1 หลังแปลเสร็จ
 * - รองรับ @a@, @aข้อความ@, @br, @strong...@ mapping ตำแหน่งกับ child node เดิม
 * - svg/img/element อื่นๆ จะอยู่ตำแหน่งเดิม
 * - ปุ่มภาษา: SVG globe ขนาด 19px + ข้อความ กึ่งกลางแนวตั้ง gap 15px
 * - **เพิ่มระบบตรวจ version/etag/hash ของไฟล์ json ทุกครั้ง ถ้าไม่ตรงกับ cache ให้ดึงใหม่**
 * - รองรับ UX, parallel, worker pool, auto-detect language, opacity gating, mutation observer
 */
...
class LanguageManager {
  ...
  async fetchWithEtagOrHash(url, cacheKey, metaKey) {
    // 1. Try get meta: { etag, hash, version }
    ...
    if (cachedHash) headers['If-None-Match'] = cachedHash; // fallback
    ...
    if (resp.status === 304) {
      // Not modified, keep cache (already in storage)
      return { updated: false };
    }
    // Try get etag/hash/version from response
    let etag = resp.headers.get('ETag');
    let version = resp.headers.get('X-Version');
    let hash = resp.headers.get('X-Hash');
    // Fallback: if not found, try to compute hash from body (SHA-256)
    ...
    // Compare version/hash/etag with meta
    if (
  ...
  async prefetchEnterprise() {
    ...
    // 1. Check local storage for config meta
    let config = null;
    ...
    // 2. Check remote file for new version/hash/etag
    const url = '/assets/json/language.min.json';
    const metaKey = 'langcfg';
    let needUpdate = true;
    if (config) {
      // Try check with HEAD request for etag/hash/version
      try {
  ...
  async enterpriseLoadLang(lang) {
    ...
    // 2. Check remote hash/version/etag
  ...
  async prepareAllButtonTexts() {
    ...
    // Add SVG globe icon (size 19px)
    const svgWrap = document.createElement('span');
    svgWrap.className = 'lang-btn-svg';
    svgWrap.innerHTML = `<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" fill="currentColor" class="bi bi-globe-americas" viewBox="0 0 16 16" style="display:block;">
    ...
    // Add all language texts (hidden except selected)
