 ...
 * - หากการแปลไม่ระบุการจัดวาง SVG ใด ๆ (ไม่มี @svg@/@lsvg@ และไม่มี <svg> ในเนื้อหาแปล) แต่มี <svg> อยู่แล้วใน DOM,
 *   ระบบจะคาดคะเนและแทรก marker แบบ internal (lsvg) ที่ตำแหน่งเริ่มต้นของเนื้อหา (front-most) — ทำให้ svg ที่มีอยู่เดิมถูกย้ายไปข้างหน้าสุดโดยอัตโนมัติ
 * - หากมีการระบุ @lsvg@ ในสตริงแปล จะยึดตำแหน่งนั้น (แต่ยังคงความพยายามในการ reuse node เดิม)
 * - ระบบ reconciliation ยังคงพยายาม preserve identity ของ <svg> (ไม่ replace element ถ้า match) เพื่อรักษา event listeners / state
 *
 * พฤติกรรมโดยย่อ:
 * - ถ้ามีข้อมูลการวาง svg ตัวใดตัวหนึ่งในสตริงแปล (เช่น @svg@, @svg:id@, @lsvg@ หรือ inline <svg>), ระบบจะยึดตามข้อมูลนั้น
 * - ถ้าไม่มีข้อมูลใด ๆ แต่ DOM มี <svg> อยู่แล้ว ระบบจะ "คาดคะเน" ว่า svg ตัวแรก (หรือที่ match ตาม id ถ้ามี) ควรไปอยู่ข้างหน้าสุด — โดยการแทรก internal lsvg marker ที่ตำแหน่งหน้าแรก
 *
    ...
    const resolveSlotMarkerGlobal = (name) => {
      if (name) {
        for (let s of existingSlotsAll) {
          ...
          if ((s.getAttribute && s.getAttribute('data-translate-slot') === name) || (s.getAttribute && s.getAttribute('data-slot') === name) || (s.dataset && s.dataset.translateSlot === name) || (s.dataset && s.dataset.slot === name)) {
            usedSlots.add(s);
            return s;
          }
        }
