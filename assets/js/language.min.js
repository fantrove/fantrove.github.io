 ...
 * เพิ่มการบังคับภาษาระดับสูง (high-assurance language enforcement)
 *
 * เป้าหมายของการอัปเดตนี้:
 * - บังคับใส่ข้อมูลภาษาให้เบราว์เซอร์รับรู้โดยเร็วที่สุด (ก่อน/ทันทีที่สคริปต์ถูกรัน)
 * - ใส่ attribute/metadata หลายชั้น (html@lang, xml:lang, body@lang, meta Content-Language, meta name=language)
 * - ใส่ lang ให้กับองค์ประกอบ semantic/landmark และ [data-translate] ทันที
 * - ตั้ง cookie/LocalStorage เพื่อให้ server/client อื่น ๆ สามารถอ่านได้สำหรับการเรียกถัดไป
 * - สร้าง rel="alternate" hreflang placeholders (จะอัปเดตเมื่อโหลด languagesConfig)
 *
 * หมายเหตุ: การกำหนด header HTTP (เช่น Content-Language) ไม่สามารถทำจากฝั่ง client สำหรับ request ปัจจุบัน
 * การตั้งค่า JS จะช่วยให้ UA / assistive tech / spellcheckers รับรู้ภาษาบนเอกสารปัจจุบันได้ดีขึ้น
 */
...
class LanguageManager {
  constructor() {
    ...
    this._selectedLang = ""; // internal storage; exposed via getter/setter
    ...
    this._earlyBootstrapDone = false;

    // selectedLang getter/setter to enforce language attributes immediately on change
    Object.defineProperty(this, 'selectedLang', {
      ...
      set: (lang) => {
        ...
        try {
          this.setEnforcedLang(lang, { reason: 'setter' });
          // persist for future loads
          try { localStorage.setItem('selectedLang', lang); } catch (e) {}
          // set cookie for server-side awareness on next request
          try { document.cookie = `site_lang=${encodeURIComponent(lang)}; path=/; SameSite=Lax`; } catch (e) {}
        } catch (e) {}
      },
      configurable: true,
      enumerable: true
    ...
    // Immediately run aggressive early bootstrap so browser can see language metadata ASAP.
    // This runs even before fetching languagesConfig; it uses localStorage/URL/navigator to pick a candidate.
    try { this.earlyBootstrap(); } catch (e) {}
  ...
  // ---------------- Early bootstrap (aggressive) ----------------
  earlyBootstrap() {
    if (this._earlyBootstrapDone) return;
    this._earlyBootstrapDone = true;

    // Determine candidate language quickly: URL lang param > localStorage.selectedLang > navigator languages
    let lang = null;
    try {
      const urlParams = (typeof window !== 'undefined' && window.location) ? new URLSearchParams(window.location.search) : null;
      if (urlParams) lang = urlParams.get('lang');
    } catch (e) {}
    try {
      if (!lang) lang = localStorage.getItem('selectedLang');
    } catch (e) {}
    try {
      if (!lang) {
        const nav = navigator.languages || [navigator.language || navigator.userLanguage];
        if (nav && nav.length) {
          lang = nav[0].split('-')[0];
        }
      }
    } catch (e) { lang = lang || 'en'; }
    if (!lang) lang = 'en';

    // Quick setter: update internal selectedLang but do not trigger full page translate yet.
    // The setter will call setEnforcedLang which is safe even if languagesConfig not loaded.
    try { this._selectedLang = lang; this.setEnforcedLang(lang, { reason: 'earlyBootstrap' }); } catch (e) {}
    // persist candidate
    try { localStorage.setItem('selectedLang', lang); } catch (e) {}
    try { document.cookie = `site_lang=${encodeURIComponent(lang)}; path=/; SameSite=Lax`; } catch (e) {}

    // Add a small inline marker script/meta so UA sees lang as soon as possible.
    try {
      const head = document.head || document.getElementsByTagName('head')[0];
      if (head) {
        // Add simple meta if not present
        if (!head.querySelector('meta[name="language"][data-langmgr="early"]')) {
          const m = document.createElement('meta');
          m.setAttribute('name', 'language');
          m.setAttribute('content', lang);
          m.setAttribute('data-langmgr', 'early');
          head.appendChild(m);
        }
        if (!head.querySelector('meta[http-equiv="Content-Language"][data-langmgr="early"]')) {
          const mc = document.createElement('meta');
          mc.setAttribute('http-equiv', 'Content-Language');
          mc.setAttribute('content', lang);
          mc.setAttribute('data-langmgr', 'early');
          head.appendChild(mc);
        }
      }
    } catch (e) {}

    // Ensure we run again when DOM becomes interactive (set attributes on semantic landmarks)
    if (document.readyState === 'loading') {
      document.addEventListener('readystatechange', () => {
        if (document.readyState === 'interactive' || document.readyState === 'complete') {
          try { this.setEnforcedLang(this._selectedLang, { reason: 'readystatechange' }); } catch (e) {}
        }
      });
    } else {
      try { this.setEnforcedLang(this._selectedLang, { reason: 'bootstrap-sync' }); } catch (e) {}
    }
  }
  ...
  // This method sets lang attributes in many places and configures direction/hreflang/meta tags
  setEnforcedLang(language, opts = {}) {
    if (!language) {
      try {
    ...
    // If languagesConfig provides a locale, use it; otherwise map some common codes to locales
    let locale = language;
    try {
      if (this.languagesConfig && this.languagesConfig[language] && this.languagesConfig[language].locale) {
        ...
        const map = {
          en: 'en-US', th: 'th-TH', es: 'es-ES', fr: 'fr-FR', de: 'de-DE',
          ja: 'ja-JP', ko: 'ko-KR', zh: 'zh-CN', 'zh-TW': 'zh-TW', ar: 'ar-SA',
          ru: 'ru-RU', pt: 'pt-BR'
        };
        if (map[language]) locale = map[language];
        if (!locale || locale.length === 2) locale = language.toLowerCase();
      }
    } catch (e) {}
    ...
    try {
      ...
      // add machine-friendly class to <html> for CSS and UA heuristics
      try {
        const cls = `lang-${locale.replace(/[^a-zA-Z0-9_-]/g,'')}`;
        if (!document.documentElement.classList.contains(cls)) document.documentElement.classList.add(cls);
      } catch (e) {}
    } catch (e) {}

    // body and semantic landmarks
    try {
      if (document.body) document.body.setAttribute('lang', locale);
      ['main','header','footer','nav','article','section','aside','form'].forEach(tag => {
        try {
          const el = document.getElementsByTagName(tag);
          if (el && el.length) {
            Array.from(el).forEach(n => { try { n.setAttribute('lang', locale); } catch(e){} });
          }
        } catch (e) {}
      });
    } catch (e) {}

    // set Content-Language meta + name=language
    try {
      let metaCL = document.head.querySelector('meta[http-equiv="Content-Language"][data-langmgr="1"]');
      if (!metaCL) {
        // try to reuse early meta (data-langmgr="early")
        metaCL = document.head.querySelector('meta[http-equiv="Content-Language"][data-langmgr="early"]');
      }
      if (!metaCL) {
      ...
      metaCL.setAttribute('content', locale);
    } catch (e) {}

    try {
      let metaLang = document.head.querySelector('meta[name="language"][data-langmgr="1"]');
      if (!metaLang) metaLang = document.head.querySelector('meta[name="language"][data-langmgr="early"]');
      if (!metaLang) {
    ...
    // set dir attribute (rtl/ltr) using languagesConfig.dir or simple primary-subtag check
    try {
      let dir = 'ltr';
    ...
    // set lang on [data-translate] and slot elements
    try {
      const transEls = document.querySelectorAll('[data-translate]');
      transEls.forEach(el => {
        try { el.setAttribute('lang', locale); } catch (e) {}
      });
      const slotEls = document.querySelectorAll('[data-translate-slot],[data-slot]');
      slotEls.forEach(el => {
        try { el.setAttribute('lang', locale); } catch (e) {}
      });
    } catch (e) {}

    // update rel="alternate" hreflang links (mark with data-langmgr)
    try {
      // remove manager-created ones
      Array.from(document.head.querySelectorAll('link[rel="alternate"][data-langmgr="1"]')).forEach(l => l.remove());
      if (this.languagesConfig && typeof window !== 'undefined') {
        Object.entries(this.languagesConfig).forEach(([code, cfg]) => {
          try {
            let hreflang = (cfg && cfg.locale) ? cfg.locale : code;
    ...
    // store ping: set cookie and localStorage for server/client handoff
    try { localStorage.setItem('selectedLang', language); } catch (e) {}
    try { document.cookie = `site_lang=${encodeURIComponent(language)}; path=/; SameSite=Lax`; } catch (e) {}

    // optionally: set a beacon to server if endpoint is present (non-blocking, best-effort)
    try {
      const beaconUrl = '/.lang-beacon';
      if (navigator && navigator.sendBeacon) {
        const payload = JSON.stringify({ lang: language, ts: Date.now(), origin: location.pathname });
        try { navigator.sendBeacon(beaconUrl, payload); } catch (e) {}
      }
    } catch (e) {}
  }
  ...
  async prefetchEnterprise() {
    ...
    if (config) {
      ...
      // after languagesConfig is available, re-run enforced lang to apply locale mapping and rel-alternate links
      try { this.setEnforcedLang(this._selectedLang, { reason: 'config-loaded' }); } catch (e) {}
    }
  ...
  async prepareAllButtonTexts() {
    ...
    svgWrap.innerHTML = `<!-- language icon --> <svg xmlns="http://www.w3.org/2000/svg" width="18.5" height="18.5" viewBox="0 0 24 24" fill="none" stroke="#000000" stroke-width="1.25" stroke-linecap="round" stroke-linejoin="round"><path d="M4 5h7" /><path d="M9 3v2c0 4.418 -2.239 8 -5 8" /><path d="M5 9c0 2.144 2.952 3.908 6.7 4" /><path d="M12 20l4 -9l4 9" /><path d="M19.1 18h-6.2" /></svg>`;
  ...
  async handleInitialLanguage() {
    ...
    // selectedLang setter enforces attributes immediately
  ...
  async updatePageLanguage(language, options = { updateHistory: true }) {
    ...
    try {
      ...
      // set via setter to enforce attributes early and persist
      this.selectedLang = language;

      // streaming translate or reset
      if (language === 'en') {
  ...
  async parallelStreamingTranslate(languageData, elements) {
    ...
    for (let j = 0; j < results.length; ++j) {
      ...
      for (let k = 0; k < resArr.length; ++k) {
        ...
        // reinforce lang attribute on element after replace
  ...
  _replaceDOMWithMarkerReplace(el, parts) {
    ...
    // If translation did NOT explicitly reference any svg/lsvg and there are existing svgs in DOM,
    // we will "predict" and put an internal lsvg marker to the very front so svg gets preserved/moved front-most.
    ...
    // anchors tracking for reuse to preserve href/attributes/eventlisteners
    ...
    // Reconcile without wiping entire element
    let readIndex = 0; // pointer in existing children
    for (let i = 0; i < newNodes.length; i++) {
      ...
      // Handle anchor reuse/preservation
      ...
      // For normal nodes: try to reuse/update existing
      if (currentOld) {
    ...
    // Remove trailing nodes that are not preserved (but keep svg and slot placeholders)
