 ...
 * @version 2.6.0
 *
 * เวอร์ชั่นนี้ได้รับการปรับปรุงให้ระบบมีประสิทธิภาพสูงขึ้น โดยการลดการเรียกใช้งาน DOM ที่ซ้ำซ้อน
 * และเพิ่มการจัดการ Promise รวมถึงการคอมไพล์ regex เพียงครั้งเดียว ซึ่งช่วยลดเวลาในการประมวลผล
 * ทั้งหมดของระบบยังคงประกาศการทำงานและฟีเจอร์เดิมไม่เปลี่ยนแปลง
 */
class LanguageManager {
  constructor() {
    ...
    this.FADE_DURATION = 300; // สำหรับ animation dropdown
    this.UPDATE_DELAY = 0;  // ไม่มี delayเพื่อให้การแปลทันที
  
    // Throttling สำหรับ MutationObserver (50ms)
    ...
    // คอมไพล์ regex ที่ใช้ใน parseDSL เพียงครั้งเดียวเพื่อเพิ่มประสิทธิภาพ
    this.dslLineRegex = /^\s*([^"\s]+)\s+"(.*)"\s*$/;
  }
  ...
  openIndexedDB() {
    return new Promise((resolve, reject) => {
      ...
      // เพิ่มการจัดการ onblocked เพื่อประสิทธิภาพที่ดีขึ้น
      request.onblocked = () => {
  ...
  async initialize() {
    ...
    try {
      ...
      // กำหนด <html> ให้ lang ว่าง เพื่อให้ระบบของเราจัดการเอง
   ...
   * จัดการภาษาเริ่มต้น
   *
   * เมื่อเริ่มต้น ระบบจะอ่านค่าภาษาที่ถูกบันทึกไว้ใน localStorage (selectedLang)
   * แล้วคงไว้ไม่ให้ย้อนกลับไปใช้ภาษาของหน้าก่อนหน้า
   */
  async handleInitialLanguage() {
    this.storeOriginalContent();
    const urlParams = new URLSearchParams(window.location.search);
    const langFromUrl = urlParams.get('lang');
    // ใช้ค่าที่บันทึกไว้ใน localStorage เป็นหลัก
  ...
  /**
   * กำหนดภาษาเริ่มต้น (ใช้ได้ในกรณีอื่น ๆ)
   */
   ...
   *
   * ระบบจะจัดการ attribute lang ของ <html> ทันทีตามที่เลือก
   * และตรวจสอบว่าภาษาที่เลือกตรงกับภาษาของเบราว์เซอร์หรือไม่
   * ถ้าตรงกัน ระบบจะเพิ่ม attribute translate="no" และ meta tag เพื่อบล็อคการแปล
   */
  async updatePageLanguage(language, options = { updateHistory: true }) {
    ...
    try {
      ...
      // ตั้งค่า attribute lang ของ <html> ทันที
      document.documentElement.setAttribute("lang", "");
      document.documentElement.setAttribute("lang", language);
      
      // ตรวจสอบ match ระหว่างภาษาที่เลือกกับภาษาของเบราว์เซอร์
      if (language === this.detectBrowserLanguage()) {
        // ถ้า match ให้เพิ่ม attribute translate="no" เพื่อบล็อคการแปลอัตโนมัติ
        document.documentElement.setAttribute("translate", "no");
        // เพิ่ม meta tag เพื่อบล็อคการแปล (เช่น สำหรับ Google Translate)
        ...
        // หากไม่ตรงกัน ให้ลบ attribute และ meta tagเพื่อให้การแปลทำงานได้ตามปกติ
  ...
  parseDSL(dslText) {
    ...
    for (const line of lines) {
      ...
      const match = trimLine.match(this.dslLineRegex);
      if (match) {
  ...
  /**
   * จัดการ navigation events โดยไม่ย้อนกลับไปใช้ภาษาของหน้าก่อน
   * ระบบจะอัปเดตเป็นภาษาที่ถูกเลือกใน localStorage เสมอ
   */
  ...
  async translatePageContent(languageData) {
    ...
    // คอมไพล์ regex สำหรับ marker @a เพียงครั้งเดียว
    const aTagRegex = /@a(.*?)@/g;
    await Promise.all(
  ...
  populateLanguageDropdown() {
    // ใช้ DocumentFragment เพื่อเพิ่มประสิทธิภาพในการ render
