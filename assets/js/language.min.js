 ...
 * @version 2.5.1
 *
 * เวอร์ชั่นนี้ได้รับการปรับปรุงให้ระบบมีการออกแบบอย่างชาญฉลาดมากขึ้น โดยที่เมื่อผู้ใช้ย้อนกลับไปยังหน้าเว็บก่อนหน้านี้
 * ซึ่งมีภาษาที่ต่างออกไป ระบบจะไม่เปลี่ยนกลับไปใช้ภาษาของหน้าก่อนหน้าอีกต่อไป แต่จะคงไว้ซึ่งภาษาที่ผู้ใช้เลือกไว้ (ผ่าน localStorage)
 *
 * เทคนิคขั้นสูง:
 * - ปรับปรุง popstate event handler เพื่อให้ใช้งานค่าภาษาใน localStorage แทนที่จะเอาค่าใน event.state มาใช้งาน
 * - การออกแบบระบบนี้ช่วยป้องกันปัญหาการ revert ไปยังภาษาเก่าเมื่อผู้ใช้ย้อนกลับในประวัติของเบราว์เซอร์
 *
 * ระบบจัดการภาษาและการเก็บข้อมูล (IndexedDB สำหรับ DSL และ JSON config) ยังคงเหมือนเดิม
 * โดย <html> จะถูกเซ็ต lang โดยระบบของเราเท่านั้น
 */
class LanguageManager {
  constructor() {
    ...
    // Throttling for MutationObserver (เล็กน้อยเพื่อไม่ให้เรียกซ้ำเกินความจำเป็น)
  ...
  async handleInitialLanguage() {
    ...
    // ใช้ค่าภาษาจาก localStorage เป็นหลัก ดังนั้นเราจะไม่ revert ไปเป็นภาษาของหน้าก่อนหน้า
  ...
  async updatePageLanguage(language, options = { updateHistory: true }) {
    ...
    try {
      ...
      // ทันทีล้างค่า lang และเซ็ตใหม่ตามที่เราเลือก
      document.documentElement.setAttribute("lang", "");
  ...
  /**
   * จัดการ navigation events โดยไม่ยอมให้ย้อนกลับไปใช้ภาษาของหน้าก่อนหน้า
   * ระบบจะอ่านภาษาปัจจุบันจาก localStorage และอัพเดทให้ทันที
   */
  setupNavigationHandlers() {
    window.addEventListener('popstate', async (event) => {
      ...
      try {
        // แทนที่จะใช้ค่า event.state, อ่านภาษาปัจจุบันที่ถูกบันทึกไว้ล่าสุด
        const storedLang = localStorage.getItem('selectedLang') || 'en';
        if (storedLang !== this.selectedLang) {
          // ถ้าเกิดมีความแตกต่าง ให้แน่ใจว่าเราทำการอัพเดทให้ตรงกับภาษาที่เลือกไว้
  ...
  async openLanguageDropdown() {
    ...
    return new Promise(resolve => {
      requestAnimationFrame(() => {
        ...
        // ใช้ FADE_DURATION สำหรับ animation dropdown เท่านั้น
