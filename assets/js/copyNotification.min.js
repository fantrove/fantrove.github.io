(function(global) {
  ...
  function removeExistingNotification() {
    ...
    if (existing) {
      existing.style.animation = 'fadeOutCopy 0.18s cubic-bezier(.8,0,.2,1) forwards';
  ...
  function showCopyNotification({ text, name, typeId = 'emoji', lang } = {}) {
    ...
    // Style (refined, more professional, smoother, larger SVG, more modern)
    if (!document.querySelector('#copy-notification-topcenter-styles')) {
      ...
      style.textContent = `
        .copy-notification-topcenter {
          ...
          backdrop-filter: blur(2.4px) saturate(1.15);
          color: #fff;
          ...
          z-index: 15000;
          opacity: 0;
          animation: slideInCopy 0.32s cubic-bezier(.7,0,.2,1.1) forwards;
          max-width: 350px;
          min-width: 120px;
          font-size: 1.09em;
          display: flex;
        ...
        .copy-icon {
          ...
          overflow: hidden;
          animation: iconPopIn 0.22s cubic-bezier(.77,-0.3,.34,1.3) forwards;
        }
        .copy-icon svg {
          width: 32px;
          height: 32px;
        ...
        .copy-tick-path {
          ...
          animation: tickDraw 0.33s cubic-bezier(.83,0,.17,1.04) 0.12s forwards;
        }
        .copy-message {
          flex: 1;
          display: flex;
        ...
        .copy-emoji {
          ...
          white-space: pre;
          filter: drop-shadow(0 1.3px 2px rgba(0,0,0,0.07));
          will-change: filter;
        ...
        @keyframes slideInCopy {
          from { opacity: 0; transform: translateX(-50%) translateY(-40px) scale(.88) translateZ(0);}
          60%  { opacity: 1; transform: translateX(-50%) translateY(6px) scale(1.03) translateZ(0);}
          90%  { opacity: 1; transform: translateX(-50%) translateY(-2px) scale(0.99) translateZ(0);}
          to   { opacity: 1; transform: translateX(-50%) translateY(0) scale(1) translateZ(0);}
        }
    ...
    notification.onclick = notification.onkeydown = (e) => {
      if (e.type === 'click' || e.key === 'Enter' || e.key === 'Escape') {
        notification.style.animation = 'fadeOutCopy 0.17s cubic-bezier(.8,0,.2,1) forwards';
    ...
    setTimeout(() => {
      if (document.body.contains(notification)) {
        notification.style.animation = 'fadeOutCopy 0.19s cubic-bezier(.8,0,.2,1) forwards';
        notification.style.pointerEvents = 'none';
        setTimeout(() => {
          if (document.body.contains(notification)) notification.remove();
        }, 190);
