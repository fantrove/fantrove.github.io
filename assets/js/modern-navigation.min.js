...
class ModernNavigation {
 ...
 constructor(config = {}) {
  ...
  this.defaultButtonClass = config.defaultButtonClass || 'default-button'; // เพิ่มคลาสเริ่มต้น
 ...
 _createNavigationItems() {
  ...
  navContainer.innerHTML = this._config.navigation.map(item => {
    // จัดการคลาสของปุ่ม
    let buttonClasses = ['nav-item'];
    
    // เพิ่ม customClass ถ้ามี หรือใช้ defaultButtonClass
    if (item.customClass) {
      buttonClasses.push(item.customClass);
    } else {
      buttonClasses.push(this.defaultButtonClass);
    }
    
    // สร้าง HTML string
    return `
      <button class="${buttonClasses.join(' ')}" data-link="${item.url}">
        ${item.icon || ''}
        <div class="label">${item[`${this.currentLang}_label`] || item.en_label || 'Missing Label'}</div>
      </button>
    `;
  }).join('');
 }
...
document.addEventListener('DOMContentLoaded', () => {
 const navigation = new ModernNavigation({
  ...
  defaultButtonClass: 'default-button' // กำหนดคลาสเริ่มต้นสำหรับปุ่ม
 });
