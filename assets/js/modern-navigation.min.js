...
class ModernNavigation {
 ...
 async init() {
  ...
  try {
   ...
   await this._loadCSS();
   this._config = await this._fetchConfigUltraSmart();
 ...
 _handleClick(event) {
  ...
  if ((goLink || targetLink) && !item.classList.contains(this.activeClass)) {
   // ตรวจสอบ active state เฉพาะ url ปกติ
   if (window.location.pathname.endsWith(targetLink)) return;
   // ถ้ามี goUrl ให้ใช้ goUrl, ถ้าไม่มีใช้ url
 ...
 // เทคนิคขั้นสูง: ultra-lightweight config refresh ใช้ HEAD+Cache API+ETag/Last-Modified, ดึงใหม่เมื่อจำเป็น
 async _fetchConfigUltraSmart() {
  // ใช้ cache API ถ้า browser รองรับ
  if ('caches' in window) {
   try {
    const cache = await caches.open('modern-nav-config');
    let cachedResponse = await cache.match(this.configPath);

    // เทคนิคขั้นสูง: ดึงแค่ header ก่อน
  ...
  // fallback: ultra-lightweight no-store fetch (ไม่ cache ฝั่ง browser)
