...
class ModernNavigation {
  ...
  _createNavigationItemsFragment(){
    ...
    for (let i = 0; i < config.length; i++){
      ...
      link.href = item.go_url || item.url || '#';
  ...
  _onNavClick(event){
    ...
    // if target is same as current path, prevent default
    if (window.location.pathname.endsWith(targetLink)) {
      event.preventDefault();
      return;
    }
    
    // Allow default anchor behavior (will navigate to href)
    // If you need custom handling, preventDefault and use window.location.assign(destination)
  }
  ...
  _updateActiveState(){
    ...
    for (let i = 0; i < items.length; i++){
      ...
      const shouldBeActive =
        (navLink === '/' || navLink === '/index.html') ?
        path === '/' || path.endsWith('/index.html') :
        path.startsWith(navLink.endsWith('/') ? navLink : `${navLink}/`);
