...
// ปรับให้ใช้งานกับ dataManager ที่ประกอบ db จาก fantrove-data/*
  ...
  try {
      ...
      function buildIndex(obj) {
        if (Array.isArray(obj)) {
          ...
          if (obj.name && typeof obj.name === 'object') {
            ...
            const nameNorm = (obj.name || "").trim().toLowerCase();
            if (nameNorm) symbolMap.set(nameNorm, obj);
          }
          for (const key in obj) {
            if (Object.prototype.hasOwnProperty.call(obj, key)) buildIndex(obj[key]);
          }
        }
      }

      try { buildIndex(db?.type || db); } catch (e) { try { buildIndex(db); } catch {} }
    ...
    function findTypeIdAndName(obj, code, parentTypeId) {
      if (Array.isArray(obj)) {
        ...
        if (obj.api === code) return { typeId: parentTypeId, name: obj.name?.[lang] || obj.name?.en || obj.api || "" };
        let newParentTypeId = parentTypeId;
        if (obj.id && obj.category && Array.isArray(obj.category)) newParentTypeId = obj.id;
        for (const key in obj) {
    ...
    if (copyInfo.api) {
      ...
      if (typeResult) { typeId = typeResult.typeId || "emoji"; name = typeResult.name; }
      else if (apiNode) name = apiNode.name?.[lang] || apiNode.name?.en || apiNode.api;
      ...
      if (!node) {
        for (let [key, value] of textMap) {
          if (key.trim().toLowerCase() === copyInfo.text.trim().toLowerCase()) { node = value; break; }
        }
        if (!node) {
          for (let [key, value] of emojiMap) {
            if (key === copyInfo.text.trim().toLowerCase()) { node = value; break; }
          }
        }
      }
    ...
    if (typeof window.showCopyNotification === "function") window.showCopyNotification(notificationParams);
    else window._headerV2_utils.showNotification(notificationParams.text, "success", { duration: 2200 });
