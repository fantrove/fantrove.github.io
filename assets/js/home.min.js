// main.js - เวอร์ชันปรับปรุงพร้อมการตรวจสอบข้อผิดพลาดโดยละเอียด
...
function getViewAllLabel(typeKey) {
  ...
  return viewAllConfigs[typeKey]?.labels?.[lang] || viewAllConfigs[typeKey]?.labels?.en || "View All";
}

function getViewAllUrl(typeKey) {
  return viewAllConfigs[typeKey]?.url || "#";
}
...
function renderHomePage(database, templates) {
  ...
  database.type.forEach(typeObj => {
    ...
    mainClone.querySelector('[data-ref="type-name"]').textContent = pickLang(typeObj.name, langList) || typeKey;
    const viewAllBtn = mainClone.querySelector('[data-ref="view-all-btn"]');
    viewAllBtn.textContent = getViewAllLabel(typeKey);
    viewAllBtn.onclick = () => { window.location.href = getViewAllUrl(typeKey); };
        ...
        card.onclick = async () => {
          if (await copyToClipboard(item.text || '')) {
            if (typeof window.showCopyNotification === 'function') {
              window.showCopyNotification({ text: item.text, name: item.api, type: typeKey });
            }
          }
        };
