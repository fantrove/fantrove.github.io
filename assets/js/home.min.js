// main.js - เวอร์ชันปรับปรุงรองรับระบบแจ้งเตือนแบบใหม่ (ใช้ typeId/ID เสมอ)

console.log("🚀 สคริปต์ home.min.js เริ่มทำงาน");
...
function getLoadDataErrorMessage(error) {
  ...
  // เพิ่มรายละเอียดของ error เพื่อการดีบัก
...
// --- การดึงข้อมูลและการแสดงผล (ปรับปรุงใหม่ทั้งหมด) ---
...
async function initializeHomepage() {
  ...
  try {
    console.log("1. กำลังเริ่มดึงข้อมูลและเทมเพลต...");
    ...
    console.log("✅ พบเทมเพลตทั้งหมด");
    
    console.log("4. เริ่มต้นการแสดงผลหน้าเว็บ...");
    renderHomePage(data, { mainTemplate, categoryTemplate, cardTemplate });
    console.log("🎉 การแสดงผลหน้าเว็บเสร็จสมบูรณ์!");
    
  } catch (error) {
    console.error("❌ เกิดข้อผิดพลาดร้ายแรงในฟังก์ชัน initializeHomepage:", error);
...
function renderHomePage(database, templates) {
  ...
  database.type.forEach(typeObj => {
    // --- ใช้ typeId (ID ของประเภท) สำหรับทุกอย่างที่เกี่ยวกับแจ้งเตือนและ viewAll ---
    const typeId = typeObj.id; // เช่น "emoji", "special-characters"
...
path1.setAttribute("d", "M3 12h12");
const path2 = document.createElementNS("http://www.w3.org/2000/svg", "path");
path2.setAttribute("d", "M11 8l4 4l-4 4");
const path3 = document.createElementNS("http://www.w3.org/2000/svg", "path");
path3.setAttribute("d", "M12 21a9 9 0 0 0 0 -18");

svgIcon.appendChild(path1);
svgIcon.appendChild(path2);
svgIcon.appendChild(path3);

// เพิ่ม margin ซ้ายเล็กน้อยให้ icon ไม่ติดข้อความ
svgIcon.style.marginLeft = "0.5em";

viewAllBtn.appendChild(svgIcon);
        ...
        card.onclick = async () => {
          if (await copyToClipboard(item.text || '')) {
            if (typeof window.showCopyNotification === 'function') {
              // ใช้ typeId เสมอ (ส่งไปที่ระบบแจ้งเตือนแบบใหม่)
              window.showCopyNotification({
                text: item.text,
                name: itemName,
                typeId: typeId, // <-- สำคัญมาก: แจ้งเตือนจะใช้ typeId
                lang: userLang
              });
            }
          }
