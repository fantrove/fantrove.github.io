...
// ส่วนที่ 6: ContentManager (Render ข้อมูลใน container เนื้อหา, animation, การ์ด/ปุ่ม)

window._headerV2_contentManager = {
    // ลบและรีเซ็ต virtual state
    async clearContent() {
        ...
        this._scrollListener && window.removeEventListener('scroll', this._scrollListener);
        this._scrollListener = null;
    },

    // Render แบบ virtual-scroll + smooth fade-in/out animation สำหรับเข้า/ออก
    async renderContent(data) {
        ...
        // --- Virtual Scrolling/Incremental Rendering Configuration ---
        ...
        this._items = items;
        this._renderedSet = renderedSet;

        let isUnmounted = false;
        this._cleanupRender = () => {
            isUnmounted = true;
            for (const node of virtualNodes) {
                if (node && node.parentNode) this._animateOutAndRemove(node, FADE_OUT_MS);
            }
        ...
        // Animate out & remove for smooth fade-out
        this._animateOutAndRemove = (node, duration) => {
            if (!node || !node.parentNode) return;
            node.style.transition = `opacity ${duration}ms cubic-bezier(.7,0,.7,1)`;
        ...
        // Render chunk utility (with fade-in)
        const renderChunk = async (count) => {
            if (isUnmounted) return;
            const fragment = document.createDocumentFragment();
            let start = renderedCount;
            let end = Math.min(renderedCount + count, items.length);
            ...
            for (let i = start; i < end; i++) {
                ...
                // Non-blocking, lazy sub-render
                if (item.group?.categoryId) {
                    this.renderGroupItems(innerContainer, item.group).then(()=>{});
                } else if (item.categoryId) {
                    this.renderGroupItems(innerContainer, { categoryId: item.categoryId, type: item.type || "button" }).then(()=>{});
                } else {
                    this.renderSingleItem(innerContainer, item).then(()=>{});
                }
                ...
                element.style.transition = `opacity ${FADE_IN_MS}ms cubic-bezier(.7,0,.7,1)`;
                fragment.appendChild(element);
                virtualNodes.push(element);
                renderedSet.add(i);
            }
            renderedCount = end;
            container.appendChild(fragment);

            // Start fade-in when in DOM
            ...
            // Limit DOM nodes for RAM: fade out old nodes, remove after transition
            while (virtualNodes.length > MAX_IN_DOM) {
                const old = virtualNodes.shift();
                this._animateOutAndRemove(old, FADE_OUT_MS);
            }

            if (renderedCount >= items.length) window._headerV2_contentLoadingManager.hide();
        };

        // Initial render: only initial chunk
        await renderChunk(INITIAL_CHUNK);
        window._headerV2_contentLoadingManager.hide();

        // Scroll trigger: lazy load more as user scrolls near bottom
        const onScroll = async () => {
            if (isUnmounted) return;
            const rect = container.getBoundingClientRect();
            const nearBottom =
                rect.bottom - window.innerHeight < NEAR_BOTTOM_PX ||
                (window.innerHeight > rect.height && renderedCount < items.length);
            if (nearBottom && renderedCount < items.length) {
        ...
        // Render more if content is short (autofill viewport)
        setTimeout(() => {
            if (container.scrollHeight <= window.innerHeight) onScroll();
        }, 0);
    },
    ...
    async renderGroupItems(container, group) {
        ...
        buttons.forEach(btn => btn && fragment.appendChild(btn));
        container.appendChild(fragment);
    },
    ...
    async createButton(config) {
        ...
        button.style.transition = `opacity 400ms cubic-bezier(.7,0,.7,1)`;
...
window._headerV2_navigationManager = {
    ...
    // Normalize various URL formats to standard ?type=...&page=...
    normalizeUrl(url) {
        if (!url) return '';
    ...
    async navigateTo(route, options = {}) {
        ...
        if (typeof route === 'object') normalizedRoute = this.normalizeUrl(route);
        ...
        try {
            ...
            // Find main/sub button configs
            const config = window._headerV2_buttonManager?.buttonConfig;
            const mainButton = config?.mainButtons.find(btn => btn.url === main || btn.jsonFile === main);
            let subButton = null;
            if (mainButton?.subButtons?.length > 0) {
                ...
                await window._headerV2_buttonManager.renderSubButtons(mainButton.subButtons, main, localStorage.getItem('selectedLang') || 'en');
            } else {
                window._headerV2_elements.subButtonsContainer.innerHTML = "";
            }
...
// ส่วนที่ 8: ButtonManager (โหลด config, render main/sub buttons, handle click, update ภาษา)
...
window._headerV2_buttonManager = {
    ...
    async handleInitialUrl(url, buttonMap, defaultButton) {
        try {
            ...
            if (!url || url === '?') {
                if (defaultButton) await this.triggerButtonClick(defaultButton.button);
                return;
            }
    ...
    async triggerButtonClick(button, options = {}) {
        if (!button) throw new Error('ไม่พบปุ่มที่จะคลิก');
        ...
        if (isSubButton) { this.state.currentSubButton = button; } else { this.state.currentMainButton = button; }
    ...
    async handleButtonClick(button, buttonConfig, isSubButton) {
        ...
        if (buttonConfig?.jsonFile) {
            const data = await window._headerV2_dataManager.fetchWithRetry(buttonConfig.jsonFile);
            await window._headerV2_contentManager.renderContent(data);
        }
    ...
    async renderSubButtons(subButtons, mainButtonUrl, lang) {
        ...
        subButtons.forEach(button => {
            ...
            subButton.addEventListener('click', async () => {
                ...
                await Promise.all([
                    window._headerV2_navigationManager.navigateTo(fullUrl),
        ...
        if (needsDefault && defaultSubButton) await this.triggerButtonClick(defaultSubButton);
    ...
    scrollActiveSubButtonIntoView(activeButton) {
        if (!activeButton || !window._headerV2_elements.subButtonsContainer) return;
        requestAnimationFrame(() => {
...
(async function headerV2_initializeApp() {
    ...
    try {
        // 1. ตรวจสอบ elements ที่จำเป็น
        if (!window._headerV2_elements.header ||
            !window._headerV2_elements.navList ||
            !window._headerV2_elements.subButtonsContainer ||
            !window._headerV2_elements.contentLoading) {
            throw new Error('ไม่พบ elements ที่จำเป็นบางส่วน');
        }
        
        // 2. เริ่มต้น Error Boundary
        window._headerV2_performanceOptimizer.setupErrorBoundary();
        
        // 3. เริ่มต้น ScrollManager (sticky nav)
        window._headerV2_scrollManager.init();
        
        // 4. เพิ่มประสิทธิภาพ
        window._headerV2_performanceOptimizer.init();
        
        // 5. Event Listener ปุ่มนำทาง
        // (ปุ่มจะถูกสร้างหลังโหลด config)
        ...
        // 7. Event URL query (popstate)
        window.addEventListener('popstate', async () => {
            try {
                let url = window.location.search;
                if (!url || url === '?') {
                    const defaultRoute = await window._headerV2_navigationManager.getDefaultRoute();
                    await window._headerV2_navigationManager.navigateTo(defaultRoute, { skipUrlUpdate: true });
                    return;
                }
                await window._headerV2_navigationManager.navigateTo(url, { skipUrlUpdate: true });
            } catch {
        ...
        // 10. โหลด config ปุ่มและ render
        await window._headerV2_buttonManager.loadConfig();
        
        appState.isInitialized = true;
        // 11. ลบ loading overlay เมื่อเนื้อหาหลักพร้อม
        ...
        window._headerV2_utils.showNotification('เกิดข้อผิดพลาดในการโหลดแอพพลิเคชัน กรุณารีเฟรชหน้าเว็บ', 'error');
        try {
            window._headerV2_performanceOptimizer.setupErrorBoundary();
        } catch {}
    }
