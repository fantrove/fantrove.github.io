...
// ส่วนที่ 6: ContentManager (เวอร์ชันเบาสุด ใช้ทรัพยากรต่ำสุด โดยไม่ลดฟังก์ชันใดๆ)
// เทคนิค: virtual fragment, async batch, minimal DOM mutation, event delegation, memory reuse, no delay, garbage collection, function complete
...
window._headerV2_contentManager = {
    ...
    async renderContent(data) {
        ...
        // เบาที่สุด: render batch เดียว, fragment, event delegation, memory reuse
        ...
        if ('requestIdleCallback' in window) {
            window.requestIdleCallback(batchRender, { timeout: 24 });
        } else {
            setTimeout(batchRender, 0);
        }

        // Scroll event สำหรับกรณี DOM ใหญ่มาก (แต่ default render หมดแล้ว)
    ...
    async createCard(cardConfig) {
        ...
        card.style.transition = `opacity 180ms cubic-bezier(.7,0,.7,1)`;
