        ...
        for (let attempt = 1; attempt <= this.constants.MAX_RETRIES; attempt++) {
            try {
                ...
                if (attempt < this.constants.MAX_RETRIES) {
                    await new Promise(res => setTimeout(res, this.constants.RETRY_DELAY));
                }
            }
        }
...
window._headerV2_contentManager = {
     ...
     * ใช้ unifiedCopyToClipboard เพื่อส่งข้อมูลไปยังระบบใหม่แทนการแสดงผลแจ้งเตือนแบบเดิม
     */
    async createButton(config) {
        ...
        // ใช้ unifiedCopyToClipboard ส่งข้อมูลไปยังระบบใหม่
        ...
        return this.animateElement(button);
    },
