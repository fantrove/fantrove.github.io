...
window._headerV2_contentManager = {
    ...
    _viewObserver: null,            // observes individual items for "view" events
    _sentinelObserver: null,        // observes sentinel to trigger next batch
    _cleanupRender: null,
    _scheduleId: null,
    _isUnmounted: false,
    ...
    _frameSamples: [],
    _avgFrameTime: 16,
    _baseBatch: 6, // larger base batch since we will render per-chunk on demand
    _minBatch: 3,
    _maxBatch: 18,
    ...
    _initLearningWorker(itemsCount = 0) {
    ...
    // Core: sentinel-driven chunked rendering.
    async renderContent(data) {
        ...
        this._initLearningWorker(data.length);

        const FADE_IN_MS = 44;
        const FADE_OUT_MS = 28;
        const MAX_IN_DOM = 28; // keep some buffer to avoid reflow thrash
        let renderedCount = 0;
        ...
        // Setup view observer (records "views" for items)
        ...
        // sentinel observer to append next chunk only when user reaches end
        ...
        // Render a batch of items synchronously but scheduled in idle if available
        const renderBatch = async (startIndex, batchSize) => {
            if (this._isUnmounted) return 0;
            ...
            if (fragment.childNodes.length > 0) {
                ...
                appended.forEach(el => {
                    try {
                        if (this._viewObserver) this._viewObserver.observe(el);
                    } catch {}
                });
            ...
            // trim excess nodes to keep DOM small
            ...
            this._recordFrameSample(duration);
            return created;
        };

        // Driver: initial render and sentinel-based next-chunk render
        const batchSize = this._computeAdaptiveBatchSize();
        // Initial render: render exactly one chunk (no further rendering until user reaches sentinel)
        const initialCount = Math.min(items.length, batchSize);
        // schedule rendering on idle for lower priority
        const schedule = (fn) => {
        ...
        // perform initial batch
        await renderBatch(0, initialCount);
        renderedCount = this._renderedSet.size;

        // append sentinel after current content
        let sentinel = createSentinel();
        container.appendChild(sentinel);

        // sentinel callback - when sentinel intersects, render next chunk only once until it's consumed
                ...
                if (entry.isIntersecting && !this._isRenderingNextBatch) {
                    ...
                    schedule(async () => {
                        try {
                            ...
                            renderedCount = this._renderedSet.size;
                            // move sentinel to end if more items remain
                            if (renderedCount < items.length) {
                                // ensure sentinel is last child
                                if (sentinel.parentNode) sentinel.parentNode.removeChild(sentinel);
                                container.appendChild(sentinel);
                            } else {
                                // all rendered -> remove sentinel and hide spinner
        ...
        if ('IntersectionObserver' in window) {
            ...
            // fallback: attach scroll listener (throttled) that checks if user scrolled near bottom of container
            const scrollHandler = this._throttledScrollCheck = this._throttledScrollCheck || (() => {
        ...
        // If no items or already finished, hide spinner
        if (renderedCount >= items.length) {
            window._headerV2_contentLoadingManager.hide();
            if (this._sentinelObserver) {
                try { this._sentinelObserver.disconnect(); } catch {}
