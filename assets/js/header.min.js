...
`;
        ...
        try {
            if (!window._headerV2_startupManager) {
                window._headerV2_startupManager = (function() {
                    const required = new Set([
                        'styles',        // critical stylesheets
                        'fonts',         // fonts readiness
                        'dataManager',   // DB assembled
                        'buttonManager', // buttons loaded
                        'managers',      // core managers (scroll, subnav etc.)
                        'navigation',    // initial navigation completed
                        'contentRender', // initial content rendered
                        'perf'           // perf optimizer ready
                    ]);
                    const ready = new Set();
                    const failed = new Set();
                    let resolveAll, rejectAll;
                    const readyPromise = new Promise((res, rej) => { resolveAll = res; rejectAll = rej; });
                    let settled = false;

                    function checkAndResolve() {
                        if (settled) return;
                        // if all required are in ready, resolve
                    ...
                    function markFailed(name) {
                        try {
                            ...
                            // don't immediately reject — let waiter decide; but mark as failed
                        } catch {}
                    }
                    ...
                    // waitForAll with timeout -> resolves when ready or timeout occurs (returns status)
            ...
            // overlay is present -> mark overlayInjected so modules can check if needed
