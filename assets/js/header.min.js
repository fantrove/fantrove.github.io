...
async clearContent() {
    ...
    // เก็บสถานะ active ไว้ก่อน
    const activeMainButton = document.querySelector('nav ul li button.active');
    const activeSubButton = document.querySelector('#sub-buttons-container button.active');
...
const ButtonManager = {
    ...
    state: {
        ...
        isInitialized: false,
        cache: new Map(),
        lastUpdate: '2025-04-17 13:15:56',
        currentUser: 'Jeffy2600-4'
    },

    // เก็บสถานะ active ของปุ่ม
    _activeStates: {
        mainButton: null,
        subButton: null
    },
    ...
    // เก็บสถานะ active
    saveActiveStates() {
        this._activeStates.mainButton = document.querySelector('nav ul li button.active');
        this._activeStates.subButton = document.querySelector('#sub-buttons-container button.active');
    },

    // คืนค่าสถานะ active
    restoreActiveStates() {
        if (this._activeStates.mainButton) {
            this._activeStates.mainButton.classList.add('active');
        }
        if (this._activeStates.subButton) {
            this._activeStates.subButton.classList.add('active');
        }
    },
    ...
    async renderMainButtons() {
        ...
        for (const button of mainButtons) {
            ...
            // Event listener สำหรับปุ่มหลัก
            mainButton.addEventListener('click', async (event) => {
                try {
                    event.preventDefault();
                    
                    // เก็บสถานะก่อนเปลี่ยนแปลง
                    this.saveActiveStates();
        ...
        // คืนค่าสถานะ active หลังจากเรนเดอร์เสร็จ
        this.restoreActiveStates();
    },
