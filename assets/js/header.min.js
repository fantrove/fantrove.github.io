...
window._headerV2_contentManager={
    _vn:[],_items:[],_rset:new Set(),_scroll:null,_cleanup:null,
    async clearContent(){
        ...
        this._vn=[];this._items=[];this._rset.clear();
    ...
    async renderContent(data){
        ...
        const INIT=12,CHK=8,MAX=60,BOT=320,FIN=400,FOUT=350;
        let rendered=0,vn=[],items=data,rset=new Set(),unmount=!1,rendering=!1;
        this._vn=vn;this._items=items;this._rset=rset;

        this._cleanup=()=>{unmount=!0;vn.forEach(n=>n&&n.parentNode&&this._animOut(n,FOUT));vn.length=0;rset.clear();window.removeEventListener("scroll",this._scroll);};
        this._animOut=(n,d)=>{if(!n||!n.parentNode)return;n.style.transition=`opacity ${d}ms cubic-bezier(.7,0,.7,1)`;n.style.opacity=0;n.classList.add("fade-out");setTimeout(()=>n.parentNode&&n.parentNode.removeChild(n),d+16);};

        const renderChunk=async count=>{
            if(unmount||rendering)return;rendering=!0;
            const frag=document.createDocumentFragment();
            for(let i=rendered;i<Math.min(rendered+count,items.length);i++){
                if(rset.has(i))continue;
                const it=items[i],el=document.createElement("div");
                el.id=it.id||`content-item-${i}`;
                const inner=this.createContainer(it);
                if(it.group?.categoryId)this.renderGroupItems(inner,it.group);
                else if(it.categoryId)this.renderGroupItems(inner,{categoryId:it.categoryId,type:it.type||"button"});
                else this.renderSingleItem(inner,it);
                el.appendChild(inner);
                el.style.opacity=0;el.classList.add("fade-in");
                el.style.transition=`opacity ${FIN}ms cubic-bezier(.7,0,.7,1)`;
                frag.appendChild(el);vn.push(el);rset.add(i);
            }
            rendered+=Math.min(count,items.length-rendered);
            c.appendChild(frag);
            requestAnimationFrame(()=>{frag.childNodes.forEach(n=>n.style.opacity=1);});
            while(vn.length>MAX)this._animOut(vn.shift(),FOUT);
            if(rendered>=items.length)window._headerV2_contentLoadingManager.hide();
            rendering=!1;
        };

        await renderChunk(INIT);
        window._headerV2_contentLoadingManager.hide();

        const onScroll=async()=>{
            if(unmount)return;
            const rect=c.getBoundingClientRect();
            if(rect.bottom-window.innerHeight<BOT||(window.innerHeight>rect.height&&rendered<items.length))
                await renderChunk(CHK);
        };
        this._scroll=onScroll;
        let ticking=!1;
        window.addEventListener("scroll",()=>{if(!ticking){window.requestAnimationFrame(()=>{onScroll();ticking=!1;});ticking=!0;}});
        setTimeout(()=>{if(c.scrollHeight<=window.innerHeight)onScroll();},0);
    ...
    async renderGroupItems(container,group){
        ...
        const btns=await Promise.all(data.map(item=>this.createButton(item)));
    ...
    createSimpleHeader(txt,c){
        ...
        h2.className="group-header-text";h2.textContent=txt;c.appendChild(h2);
        return c;
    },
    createHeaderComponents(c,cfg,lang){
        if(cfg.icon)c.appendChild(this.createHeaderIcon(cfg.icon));
        const hc=document.createElement("div");
        hc.className="header-content";hc.appendChild(this.createHeaderTitle(cfg,lang));
    ...
    addLangListener(c,cfg){
        if(!c._langListenerBound){window.addEventListener("languageChange",e=>this.updateHeaderLang(c,cfg,e.detail.language));c._langListenerBound=!0;}
    },
    ...
    async renderSingleItem(container,it){
        if(it.categoryId){await this.renderGroupItems(container,{categoryId:it.categoryId,type:it.type||"button"});return;}
        const e=it.type==="button"?await this.createButton(it):await this.createCard(it);
        if(e)container.appendChild(e);
    },
    async createButton(cfg){
        const b=document.createElement("button");b.className="button-content";
        ...
        try{
            if(api){
                ...
                function findApiNode(o,code){
                    ...
                    if(o&&typeof o==="object"){if(o.api===code)return o;return Object.keys(o).map(k=>findApiNode(o[k],code)).find(Boolean);}
                }
                const nd=findApiNode(db,api);
                if(nd){fc=nd.text;type=type||(nd.api?"emoji":"symbol");}
                else fc=api;
            }else if(cfg.content){fc=cfg.content;type="symbol";}
            else if(cfg.text){fc=cfg.text;type="symbol";}
            else throw"ต้องระบุ api, content หรือ text สำหรับ button";
            b.textContent=fc;
        }catch{b.textContent="Error";}
        b.addEventListener("click",async()=>{try{await unifiedCopyToClipboard({text:fc,api,type,name:api?`${api}`:""});}catch{window._headerV2_utils.showNotification("Copy failed","error");}});
        b.classList.add("fade-in");b.style.opacity=0;
    ...
    async createCard(cfg){
        ...
        const cdiv=document.createElement("div");cdiv.className="card-content";
        const tdiv=document.createElement("div");tdiv.className="card-title";
        if(typeof cfg.title==="object"){
        ...
        const ddiv=document.createElement("div");ddiv.className="card-description";
        ...
        cdiv.appendChild(ddiv);card.appendChild(cdiv);
        ...
        card.classList.add("fade-in");card.style.opacity=0;
    ...
    updateCardsLanguage(lang){
        document.querySelectorAll(".card").forEach(card=>{
            ...
            if(t){const nt=t.dataset[`title${lang.toUpperCase()}`];if(nt)t.textContent=nt;}
            const d=card.querySelector(".card-description");
            if(d){const nd=d.dataset[`desc${lang.toUpperCase()}`];if(nd)d.textContent=nd;}
            const img=card.querySelector(".card-image");
            if(img){const na=img.dataset[`alt${lang.toUpperCase()}`];if(na)img.alt=na;}
        });
    }
};
