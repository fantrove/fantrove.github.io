...
async navigateTo(route, options = {}) {
 ...
 try {
  ...
  let isValidUrl = await this.validateUrl(route);
  if (!isValidUrl) {
  ...
  if (mainButton) {
   ...
   // สำคัญ: โหลดและแสดงปุ่มย่อยทั้งหมดเสมอถ้ามี
   if (mainButton.subButtons) {
    ...
    // ถ้ามี subRoute ให้เลือกปุ่มย่อยที่ตรงกัน
    if (subRoute) {
...
// ปรับปรุงฟังก์ชัน changeURL ใน NavigationManager
async changeURL(url, force = false) {
    try {
        if (!url) return;
        const newUrl = url.includes('#') ? url : `#${url}`;
        
        // ใช้ history.pushState เพื่อบันทึกประวัติการนำทาง
    ...
    scrollActiveButtonsIntoView() {
        try {
            // ตรวจสอบการมีอยู่ของ elements ก่อนใช้งาน
            ['nav ul', '#sub-buttons-container'].forEach(selector => {
                const container = document.querySelector(selector);
                if (!container) return; // ถ้าไม่พบ element ให้ข้ามไป
                
                const activeButton = container.querySelector('button.active');
                if (!activeButton) return; // ถ้าไม่พบปุ่มที่ active ให้ข้ามไป
    ...
    // เพิ่มฟังก์ชันสำหรับตรวจสอบสถานะ elements
    validateElements() {
...
async handleInitialUrl(url, buttonMap, defaultButton) {
  try {
   ...
   if (mainButtonData) {
    ...
    // ตรวจสอบกรณีมี subRoute
    if (subRoute && config.subButtons) {
     await new Promise(resolve => setTimeout(resolve, 100));
     
     // ค้นหา sub-button ที่ตรงกับ URL
     const subButtonConfig = config.subButtons.find(sub =>
      sub.url === subRoute || sub.jsonFile === subRoute
     );
     
     if (subButtonConfig) {
      // ถ้าพบ sub-button ที่ตรงกับ URL ให้ใช้ตัวนั้นเลย
      const subButton = document.querySelector(`button[data-url="${url}"]`);
      if (subButton) {
       await this.triggerButtonClick(subButton);
      }
     } else {
      // ถ้าไม่พบ sub-button ที่ตรงกับ URL จึงค่อยใช้ default
      await this.handleDefaultSubButton(config);
     }
    } else if (!subRoute && config.subButtons) {
     // กรณีมีปุ่มย่อยแต่ไม่ได้ระบุ subRoute มา ให้ใช้ default
     await this.handleDefaultSubButton(config);
    }
    return;
   }
   
   // กรณี URL ไม่ถูกต้อง ให้ใช้ปุ่ม default
   if (defaultButton) {
    await this.triggerButtonClick(defaultButton.button);
   }
...
async triggerButtonClick(button, options = {}) {
 ...
 // แก้ไขฟังก์ชัน handleDefaultSubButton
 async handleDefaultSubButton(config) {
  ...
  // เพิ่มฟังก์ชันใหม่สำหรับตรวจสอบความถูกต้องของ URL ปุ่มย่อย
