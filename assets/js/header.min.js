...
async navigateTo(route, options = {}) {
    ...
    try {
        ...
        // ส่วนเพิ่มเติม: ดึงข้อมูลเนื้อหาที่เกี่ยวข้องไม่ว่าจะเป็นการคลิกหรือการย้อนกลับ
        try {
            if (state.buttonConfig) {
                // ค้นหาปุ่มหลักที่ตรงกับ mainRoute
                const mainButton = state.buttonConfig.mainButtons.find(btn => btn.url === mainRoute || btn.jsonFile === mainRoute);
                if (mainButton) {
                    // หากมีไฟล์ JSON สำหรับปุ่มหลัก ให้ดึงข้อมูลมาแสดงผล
                    ...
                    // หากมี subRoute และปุ่มหลักมีปุ่มย่อย
                    if (subRoute && mainButton.subButtons) {
                        const subButton = mainButton.subButtons.find(btn => btn.url === subRoute || btn.jsonFile === subRoute);
                        if (subButton && subButton.jsonFile) {
                            const subData = await DataManager.fetchWithRetry(subButton.jsonFile);
            ...
            console.error('เกิดข้อผิดพลาดในการดึงข้อมูลเนื้อหา:', fetchError);
            utils.showNotification('เกิดข้อผิดพลาดในการดึงข้อมูลเนื้อหา', 'error');
        }
    } catch (error) {
...
const initializeApp = async () => {
    ...
    try {
        ...
        const setupUrlHandling = () => {
            // จัดการการเปลี่ยนแปลง URL ด้วย History API
            window.addEventListener('popstate', () => {
