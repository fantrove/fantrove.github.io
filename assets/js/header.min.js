...
// NOTE: Replacing only three internal sections to stop using scroll-driven rendering and to reduce resource usage.
// Sections replaced: _headerV2_contentManager, _headerV2_scrollManager, _headerV2_performanceOptimizer
// Rest of file remains unchanged.

// ----------------- ContentManager (no rendering on scroll) -----------------
window._headerV2_contentManager = {
    ...
    async clearContent() {
        ...
        if (typeof this._cleanupRender === 'function') this._cleanupRender();
        this._cleanupRender = null;
    ...
    _getPriorityScoresFor(ids) {
        return new Promise((resolve) => {
            ...
            if (this._learningWorker) {
                const timer = setTimeout(() => resolve(fallback), 50);
                const onmsg = (e) => {
    ...
    async renderContent(data) {
        ...
        this._initLearningWorker();

        const FADE_IN_MS = 44;
        const FADE_OUT_MS = 28;
        const MAX_IN_DOM = 22;
        let renderedCount = 0;
        let virtualNodes = this._virtualNodes;
        let items = data.slice();
        let renderedSet = this._renderedSet;
        this._isUnmounted = false;
        ...
        if (!document.getElementById('gpu-accel-style')) {
            ...
            style.textContent = `
                .fade-in, .fade-out, .card, .button-content {
                    will-change: opacity, transform;
                    backface-visibility: hidden;
                    transform: translateZ(0);
                }
                .fade-in { opacity: 0; transition: opacity ${FADE_IN_MS}ms cubic-bezier(.7,0,.7,1) !important; }
                .fade-out { opacity: 0; transition: opacity ${FADE_OUT_MS}ms cubic-bezier(.7,0,.7,1) !important; }
            `;
            document.head.appendChild(style);
        }
        ...
        const progressiveRender = async (idleDeadline) => {
            if (this._isUnmounted) return;
            const batchSize = this._computeAdaptiveBatchSize();
            const fragment = document.createDocumentFragment();
            let batch = 0;
            const startLoop = performance.now();

            while (renderedCount < items.length && batch < batchSize) {
                const item = items[renderedCount];
                if (renderedSet.has(renderedCount)) { renderedCount++; continue; }

                const wrapper = this._acquireFromPool();
                wrapper.id = item.id || `content-item-${renderedCount}`;
                ...
                try { this._recordEvent('view', wrapper.id || item.id); } catch {}

                virtualNodes.push(wrapper);
                renderedSet.add(renderedCount);
                renderedCount++;
                batch++;
            }
            ...
            if (fragment.childNodes.length > 0) {
                ...
                requestAnimationFrame(() => {
                    for (const node of fragment.childNodes) node.style.opacity = 1;
                });
            }
            ...
            this._recordFrameSample(endLoop - startLoop);
            ...
            // observe latest appended children efficiently (IO is native and non-blocking)
            if (this._intersectionObserver) {
                const els = container.querySelectorAll('div[id]');
                els.forEach(el => {
                    try { this._intersectionObserver.observe(el); } catch {}
                });
            }

            if (renderedCount < items.length) {
                if ('requestIdleCallback' in window) {
                    this._scheduleId = window.requestIdleCallback(progressiveRender, { timeout: 100 });
                } else {
                    const delay = Math.min(40, Math.max(8, Math.round(this._avgFrameTime)));
                    this._scheduleId = setTimeout(() => progressiveRender && progressiveRender(), delay);
                }
            } else {
                window._headerV2_contentLoadingManager.hide();
            }
        ...
        if ('requestIdleCallback' in window) {
            ...
            requestAnimationFrame(() => progressiveRender && progressiveRender());
        }

        // IMPORTANT: Do not attach rendering logic to scroll events.
        // We avoid heavy scroll listeners entirely to prevent jank.
        // Visibility tracking is handled via IntersectionObserver above.
    },
...
// ----------------- ScrollManager (lightweight, avoid heavy global mutation/resize observations) -----------------
window._headerV2_scrollManager = {
    ...
    setupEventListeners() {
        // Minimal scroll handling: use passive listener and lightweight rAF throttle
    ...
    setupResizeObserver() {
        // Only observe sub-nav if present; debounce reactions
    ...
    setupMutationObserver() {
        // Avoid observing entire document: only observe insertion/removal around header/sub-nav
        try {
            const subNav = document.getElementById('sub-nav');
            if (!subNav) return;
...
// ----------------- PerformanceOptimizer (defer + limit prefetch, safer observers) -----------------
window._headerV2_performanceOptimizer = {
    ...
    setupPrefetching() {
        // Defer heavy prefetch work to idle time and limit number of prefetch links to avoid saturating network
        if (!('requestIdleCallback' in window)) {
            // fall back to small timeout
            setTimeout(() => this._doPrefetch(), 2000);
        } else {
