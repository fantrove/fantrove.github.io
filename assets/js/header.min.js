...
const NavigationManager = {
    ...
    normalizeUrl(url) {
        // url = ?type=main__&page=sub หรือ object {type, page}
        if (!url) return '';
        if (typeof url === 'object') {
            return `?type=${(url.type || '').toLowerCase()}__&page=${(url.page || '').toLowerCase()}`;
        }
        if (url.startsWith('?')) return url;
        // เดิมรับ main-sub → แปลง
        if (url.includes('-')) {
            const [main, sub] = url.split('-');
            return `?type=${main}__&page=${sub || ''}`;
        }
        // ถ้าเป็น main อย่างเดียว
        return `?type=${url}__`;
    },

    parseUrl() {
        // ดึงค่าจาก window.location.search
        const params = new URLSearchParams(window.location.search);
        const type = params.get('type');
        const page = params.get('page');
        // type อาจจะมี __ ต่อท้าย ให้ตัดออก
        return {
    ...
    async changeURL(url, force = false) {
        // เปลี่ยน window.location.search
        try {
            if (!url) return;
    ...
    async navigateTo(route, options = {}) {
        ...
        try {
            ...
            const { main, sub } = typeof route === 'object' ? route :
                route.startsWith('?') ? (() => {
    ...
    async loadMainAndSubParallel(mainButton, subButton) {
        // โหลดเนื้อหาหลักและย่อยพร้อมกัน (สำหรับ UI ที่โหลดเร็วขึ้น)
        // mainButton, subButton คือ config ปุ่ม
        try {
            const jobs = [];
