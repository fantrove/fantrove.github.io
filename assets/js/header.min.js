...
const utils = {
    ...
    // เก็บ loading notifications
    loadingNotifications: new Map(),

    // ระบบจัดการข้อความหลายภาษา
    messages: {
        th: {
            success: '✨ สำเร็จ!',
            error: '❌ ข้อผิดพลาด',
            ...
            loading: '⌛ กำลังโหลด',
            copied: 'คัดลอกแล้ว:',
            networkError: 'ขาดการเชื่อมต่ออินเทอร์เน็ต',
            networkRestore: 'กลับมาออนไลน์แล้ว',
            retry: 'ลองใหม่',
            close: 'ปิด'
        },
        en: {
     ...
     * @param {string} type - ประเภท (success, error, warning, info, loading)
    ...
    showNotification(message, type = 'info', options = {}) {
        ...
        try {
            // กรณีเป็น loading notification
            if (type === 'loading') {
                const loadingKey = options.key || 'default-loading';
                const existingLoading = this.loadingNotifications.get(loadingKey);

                if (existingLoading) {
                    // อัพเดทข้อความและรีเซ็ตเวลา
                    const messageElement = existingLoading.element.querySelector('.notification-content');
                    if (messageElement && existingLoading.message !== message) {
                        messageElement.textContent = message;
                        existingLoading.message = message;
                    }

                    // รีเซ็ตเวลาถ้ามีการกำหนด duration
                    if (options.duration && options.duration !== Infinity) {
                        if (existingLoading.timeout) {
                            clearTimeout(existingLoading.timeout);
                        }
                        existingLoading.timeout = setTimeout(() => {
                            this.removeNotification(loadingKey);
                        }, options.duration);
                    }

                    return existingLoading.element;
                }
            }

            // ลบ notification เก่าถ้ามีการระบุ key (สำหรับ non-loading notifications)
            if (options.key && type !== 'loading') {
                this.removeNotification(options.key);
            }
            ...
            messageContainer.innerHTML = `
                <div class="notification-title">${messages[type]}</div>
            ...
            // เพิ่ม notification เข้า DOM
            document.body.appendChild(notification);

            // จัดการกับ loading notifications
            if (type === 'loading') {
                const loadingKey = options.key || 'default-loading';
                let timeout = null;
                ...
                this.loadingNotifications.set(loadingKey, {
                    element: notification,
                    message: message,
                    timeout: timeout
                });
            } else if (options.duration !== Infinity) {
                // ตั้งเวลาลบสำหรับ non-loading notifications
                setTimeout(() => {
                    this.removeNotification(options.key);
                }, options.duration || this.CONSTANTS.NOTIFICATION_DURATION);
            }
    ...
    /**
     * ลบ notification
     * @param {string|Element} key - key หรือ element ของ notification
     */
    removeNotification(key) {
        if (!key) return;

        const notification = key.classList ? 
            key : // ถ้าส่ง element มาโดยตรง
            document.querySelector(`.notification[data-key="${key}"]`);

        if (notification) {
            // ลบจาก loadingNotifications ถ้ามี
            this.loadingNotifications.forEach((value, loadingKey) => {
                if (value.element === notification) {
                    if (value.timeout) {
                        clearTimeout(value.timeout);
                    }
                    this.loadingNotifications.delete(loadingKey);
                }
            });
     ...
     * @param {string} content - ข้อความที่จะคัดลอก
     */
    async copyToClipboard(content) {
        ...
        try {
            ...
            this.showNotification(`${messages.copied} ${content}`, 'success', {
                duration: 2000,
                key: 'clipboard-copy'
            });
        } catch (error) {
     ...
     * @param {number} wait - เวลาที่จะรอ (ms)
     */
     ...
     * @param {number} limit - เวลาขั้นต่ำระหว่างการเรียก (ms)
     */
    ...
    /**
     * สร้าง styles สำหรับ notifications
     */
...
// เพิ่ม Event Listeners สำหรับจัดการสถานะ offline/online
window.addEventListener('offline', () => {
    utils.showNotification(
        utils.messages[localStorage.getItem('selectedLang') || 'en'].networkError,
        'warning',
        { duration: Infinity, key: 'network-status' }
    );
});

window.addEventListener('online', () => {
    utils.showNotification(
        utils.messages[localStorage.getItem('selectedLang') || 'en'].networkRestore,
        'success',
        { duration: 3000, key: 'network-status' }
    );
});
...
addToHistory(url, type = 'user') {
 ...
 /**
 * ปรับปรุงฟังก์ชัน navigateTo และระบบบันทึกประวัติให้มีคุณภาพและความแม่นยำสูงขึ้น
 * - ดักจับทุกกรณีของ URL ทั้ง main และ sub route พร้อมตรวจสอบความซ้ำซ้อนของประวัติ
 * - ป้องกันการบันทึกประวัติซ้ำเมื่อมีการคลิกเปลี่ยนแปุ่มใหม่หรือย้อนกลับไปจนถึงประวัติแรกแล้วคลิกย้อนกลับอีก
 */
async navigateTo(route, options = {}) {
 ...
 try {
  ...
  // ตรวจสอบความถูกต้องของ URL ด้วย validateUrl()
  ...
  // แยก main route กับ sub route จาก URL
  ...
  // อัพเดท URL หากไม่ได้ระบุให้ข้ามการอัพเดท URL
  ...
  // ตรวจสอบการเปลี่ยนแปลงของ main และ sub route
  const mainChanged = previousMainRoute !== newMainRoute;
  const subChanged = previousSubRoute !== newSubRoute;
  
  // ควบคุมเงื่อนไขการบันทึกประวัติ (history)
  // สำหรับกลุ่มที่มีปุ่มย่อย (เนื้อหามาจาก sub button) ถ้าเปลี่ยนไปเลือก default แล้วไม่ต้องบันทึกประวัติซ้ำ
  let shouldRecordHistory = true;
  
  if (mainButton.subButtons && mainButton.subButtons.length > 0) {
   // หาก main route เปลี่ยน หรือไม่มี sub route ให้ใช้งาน default sub-button
   if (mainChanged || !newSubRoute) {
    ...
    // บันทึกประวัติเฉพาะครั้งเดียวสำหรับกลุ่มที่มีปุ่มย่อยแบบ default
   ...
   // กรณีปุ่มหลักไม่มีปุ่มย่อย ให้โหลดเนื้อหาของ main button ตามปกติ
   if (mainChanged) {
    try {
     await ContentManager.clearContent();
  ...
  // บันทึกประวัติการนำทาง (history) แบบ deduplicate
  if (shouldRecordHistory) {
   try {
    this.addToHistory(route);
   } catch (historyError) {
    console.error("การบันทึกประวัติการนำทางเกิดข้อผิดพลาด:", historyError);
   }
  }
  
  // เลื่อนหน้า given scroll top if not maintainScroll
...
const DataManager = {
 // ค่าคงที่สำหรับการตั้งค่าต่างๆ
 constants: {
  FETCH_TIMEOUT: 8000, // เพิ่มเป็น 8 วินาที
  RETRY_DELAY: 2000, // เพิ่มเป็น 2 วินาที
  MAX_RETRIES: 3,
  ...
  for (let i = 0; i <= this.constants.MAX_RETRIES; i++) {
   try {
    ...
    if (i < this.constants.MAX_RETRIES) {
     // แสดงข้อความกำลังลองใหม่
     utils.showNotification(
      `กำลังลองใหม่... (ครั้งที่ ${i + 1}/${this.constants.MAX_RETRIES})`,
      'info',
 ...
 async fetchBatched(urls, options = {}) {
  ...
  for (const batch of batches) {
   try {
    const batchResults = await Promise.all(
     batch.map(async url => {
      const result = await this.fetchWithRetry(url, {
       ...options,
       loadingMessage: `กำลังโหลด ${++completed}/${total}`
      });
      return result;
     })
    );
    ...
    utils.showNotification(
     'เกิดข้อผิดพลาดในการโหลดบางส่วน แต่จะดำเนินการต่อ',
     'warning'
    );
   }
  }
 ...
 subscribeToEvents(url, callbacks = {}) {
  if (!this.isOnline()) {
   callbacks.onError?.(new Error('ไม่มีการเชื่อมต่ออินเทอร์เน็ต'));
   return { unsubscribe: () => {} };
  }
  ...
  eventSource.onerror = (error) => {
   ...
   // ลองเชื่อมต่อใหม่อัตโนมัติถ้ายังออนไลน์อยู่
   if (this.isOnline()) {
    setTimeout(() => {
     this.subscribeToEvents(url, callbacks);
    }, this.constants.RETRY_DELAY);
   }
  };
...
const ButtonManager = {
    ...
    async handleInitialUrl(url, buttonMap, defaultButton) {
        try {
            if (!url) {
                // ถ้าไม่มี URL ให้ใช้ปุ่ม default
                if (defaultButton) {
            ...
            if (mainButtonData) {
                ...
                if (isValidUrl) {
                    // ถ้า URL ถูกต้อง ให้ไปที่ route นั้น
                    ...
                    // ถ้า URL ไม่ถูกต้อง ให้ใช้ default
                    if (config.subButtons) {
            ...
            // ถ้าไม่พบปุ่มหลัก ให้ใช้ปุ่ม default
            if (defaultButton) {
                await this.triggerButtonClick(defaultButton.button);
            }
        } catch (error) {
    ...
    async triggerButtonClick(button, options = {}) {
        if (!button) {
            throw new AppError('ไม่พบปุ่มที่จะคลิก', 'button-click');
        }

        try {
            if (typeof button.click !== 'function') {
                throw new AppError('ไม่สามารถคลิกปุ่มได้', 'button-click');
            }
            ...
            await Promise.resolve(button.dispatchEvent(clickEvent));
        } catch (error) {
            throw new AppError('เกิดข้อผิดพลาดในการคลิกปุ่ม', 'button-click', error);
        }
    },
    ...
    async handleDefaultSubButton(config) {
        ...
        if (defaultSubButton) {
            // ตรวจสอบว่าเป็น URL ที่ถูกต้องหรือไม่ก่อนใช้ default
            const fullUrl = `${config.url}-${defaultSubButton.url || defaultSubButton.jsonFile}`;
            const isValidUrl = await this.validateSubButtonUrl(config, fullUrl);
    ...
    async validateSubButtonUrl(config, fullUrl) {
        try {
            const [mainRoute, subRoute] = fullUrl.split('-');

            // ตรวจสอบว่า subRoute มีอยู่ในรายการปุ่มย่อยหรือไม่
            return config.subButtons.some(sub =>
                sub.url === subRoute || sub.jsonFile === subRoute
            );
        } catch (error) {
            console.error('Error validating sub-button URL:', error);
            return false;
        }
    },
    ...
    async renderSubButtons(subButtons, mainButtonUrl, lang) {
        ...
        // ตรวจสอบว่าจำเป็นต้องเรนเดอร์ใหม่หรือไม่
        const currentMainRoute = NavigationManager.state.currentMainRoute;
        const previousMainRoute = NavigationManager.state.previousMainRoute;

        // ถ้าปุ่มหลักไม่เปลี่ยน และมีปุ่มย่อยแสดงอยู่แล้ว ให้ข้ามการเรนเดอร์ใหม่
        if (currentMainRoute === previousMainRoute && subButtonsContainer.children.length > 0) {
            return;
        }
        ...
        subButtons.forEach(button => {
            ...
            subButton.addEventListener('click', async () => {
                try {
                    subButtonsContainer.querySelectorAll('.button-sub')
            ...
            // ถ้า URL ตรงกับปุ่มย่อยนี้ ให้ทำเป็น active
        ...
        // สำคัญ: ตรวจสอบว่าต้องใช้ default หรือไม่
