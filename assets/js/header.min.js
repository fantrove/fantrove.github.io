...
async function unifiedCopyToClipboard(copyInfo = {}) {
    ...
    try {
        ...
        if (copyInfo.api) {
            ...
            if (apiNode) {
                // แสดงรหัส API ในวงเล็บหลังข้อมูล
                window.showCopyNotification({
                    text: apiNode.text,
                    name: `${apiNode.api}`,
                    type: (copyInfo.type || 'emoji'),
                    lang
                });
                return;
            }
        }
        // กรณีไม่มี api หรือหาไม่เจอ
        window.showCopyNotification({
            text: copyInfo.text,
            name: copyInfo.api ? `(${copyInfo.api})` : (copyInfo.name || ''),
            type: (copyInfo.type || 'symbol'),
            lang
        });
    } catch (error) {
...
const NavigationManager = {
    ...
    /**
     * ฟังก์ชันใหม่: โหลดข้อมูล main/sub พร้อมกันแบบ parallel
     */
    async loadMainAndSubParallel(mainButton, subButton) {
        ...
        try {
            // โหลดข้อมูลทั้ง main และ sub พร้อมกัน
            const [mainData, subData] = await Promise.all([
                mainButton.jsonFile ? DataManager.fetchWithRetry(mainButton.jsonFile) : Promise.resolve(null),
                subButton && subButton.jsonFile ? DataManager.fetchWithRetry(subButton.jsonFile) : Promise.resolve(null)
            ]);
            // render ทั้ง main และ sub (ต่อกันใน container เดียว)
            if (mainData) await ContentManager.renderContent(mainData);
            if (subData) await ContentManager.renderContent(subData);
        } catch (error) {
            ContentLoadingManager.hide();
            utils.showNotification('เกิดข้อผิดพลาดในการโหลดเนื้อหา', 'error');
            throw error;
        }
        ContentLoadingManager.hide();
    },
    ...
    /**
     * นำทางไปยัง route ที่กำหนด โดยโหลด main/sub พร้อมกันถ้ามี
     */
    async navigateTo(route, options = {}) {
        ...
        try {
            ...
            if (mainButton.subButtons && mainButton.subButtons.length > 0) {
                // หา sub button จาก route (หรือ default)
            ...
            // เลื่อนหน้าจอถ้าไม่ได้ระบุ maintainScroll
...
const ContentManager = {
    ...
    async renderSingleItem(container, item) {
        ...
        /**
         * สร้างปุ่ม (button) ที่สามารถ copy ข้อความได้ (ปรับปรุงใหม่)
         */
    async createButton(config) {
        ...
        button.addEventListener('click', async () => {
            await unifiedCopyToClipboard({
                ...
                api: apiCode,
                type,
                // ส่ง name เป็น (รหัส API) แทนชื่อ
...
const DataManager = {
    ...
    async fetchWithRetry(url, options = {}) {
        ...
        for (let attempt = 1; attempt <= this.constants.MAX_RETRIES; attempt++) {
            try {
                ...
                // Parallel prefetch for speedup (fetch and HEAD in parallel)
                ...
                // Optional: pre-warm DNS/connection in parallel
                ...
                if (response.body && window.TextDecoder) {
                    ...
                    // ใช้ Web Worker ถ้าไฟล์ใหญ่ >2MB (off-main-thread, ไม่ block UI)
                    let data;
     ...
     * (หา key "api" ที่ตรงกับ apiCode ใน json ทั้งไฟล์)
     */
    async fetchApiContent(apiCode) {
        try {
            ...
            // แปลงเป็น string แล้วใช้ regex หา api ทั้งไฟล์
            // หรือวน loop หาทุก object ที่มี key api == apiCode

            // ฟังก์ชันค้นหาแบบ recursive ทั้ง object/array
            function findApiValue(obj, targetApi) {
                if (Array.isArray(obj)) {
                    ...
                    if (obj.api === targetApi) {
                        // คืนค่า text ถ้ามี, ถ้าไม่มีคืนค่า object ทั้งก้อน
                        return obj.text || obj;
                    }
                    for (const key in obj) {
                        if (Object.prototype.hasOwnProperty.call(obj, key)) {
    ...
    async fetchBatched(urls, options = {}) {
        // ดึงข้อมูลแบบ batch/concurrent เพื่อความเร็วสูงสุด
        const results = [];
        const batches = [];
