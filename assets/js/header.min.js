...
// ปรับปรุง: เพิ่มเสถียรภาพ, ซิงค์ state, การ fade-in/fade-out สมูธ, ป้องกัน glitch, lazy rendering, และไม่ block UI
...
window._headerV2_contentManager = {
    ...
    // สมูธ: render ทีละ chunk, fade-in/fade-out ด้วย CSS, ไม่ block UI
    async renderContent(data) {
        ...
        const renderChunk = async (count) => {
            if (isUnmounted || isRendering) return;
            isRendering = true;
            ...
            for (let i = start; i < end; i++) {
                ...
                element.id = item.id || `content-item-${i}`;
