...
const utils = {
    ...
    showNotification(message, type = 'info', options = {}) {
        try {
            ...
            // เพิ่ม animation
            this.animateNotification(notification, animationDuration);
            ...
            console.error('เกิดข้อผิดพลาดในการแสดง notification:', error);
        }
    },
...
const NavigationManager = {
    ...
    state: {
        ...
        isInitialLoad: true,
        previousUrl: '',
        previousMainRoute: '', // เพิ่มตัวแปรเก็บปุ่มหลักก่อนหน้า
    ...
    async validateUrl(url) {
        ...
        try {
            ...
            if (subRoute) {
                ...
                // ตรวจสอบว่า subRoute ถูกต้องหรือไม่
    ...
    // นำทางไปยัง route ที่กำหนด
    async navigateTo(route, options = {}) {
        if (this.state.isNavigating) {
            // เก็บการนำทางที่รอดำเนินการ
            this.state.pendingNavigation = { route, options };
        ...
        try {
            ...
            this.state.previousMainRoute = this.state.currentMainRoute;
            ...
            const mainButton = state.buttonConfig.mainButtons.find(btn =>
                btn.url === mainRoute || btn.jsonFile === mainRoute
            );
            
            if (mainButton) {
                // ตรวจสอบว่าต้องล้าง content หรือไม่
                const shouldClearContent = this.state.previousMainRoute !== mainRoute;
                ...
                if (mainButton.jsonFile) {
                    const data = await DataManager.fetchWithRetry(mainButton.jsonFile);
                    await ContentManager.renderContent(data);
                }
                
                // ตรวจสอบว่าต้องแสดงปุ่มย่อยใหม่หรือไม่
                if (mainButton.subButtons && this.state.previousMainRoute !== mainRoute) {
            ...
            // ตรวจสอบว่ามีการนำทางที่รอดำเนินการหรือไม่
            if (this.state.pendingNavigation) {
                const { route, options } = this.state.pendingNavigation;
                this.state.pendingNavigation = null;
                await this.navigateTo(route, options);
            }
        }
    },
    ...
    async updateButtonStates(route) {
        try {
            ...
            await this.scrollActiveButtonsIntoView();
    ...
    // เลื่อนปุ่มที่ active เข้ามาในมุมมอง
    async scrollActiveButtonsIntoView() {
        try {
...
const setupUrlHandling = () => {
 window.addEventListener('popstate', async (event) => {
  try {
   ...
   if (NavigationManager.state.isNavigating) return;
   
   const currentHash = window.location.hash.replace('#', '');
   if (!currentHash) return; // ถ้าไม่มี hash ให้ข้าม
   
   // แยก route หลักและ route ย่อย
   const [mainRoute, subRoute] = currentHash.split('-');
   
   // ตรวจสอบว่า buttonConfig พร้อมใช้งานหรือไม่
   if (!state.buttonConfig) {
    // ถ้ายังไม่มี buttonConfig ให้โหลดใหม่
    await ButtonManager.loadConfig();
   }
   
   // ตรวจสอบอีกครั้งหลังโหลด
   if (!state.buttonConfig) {
    throw new Error('ไม่สามารถโหลดการตั้งค่าปุ่มได้');
   }
   
   // หาข้อมูลปุ่มหลักจาก config
   const mainButton = state.buttonConfig.mainButtons?.find(btn =>
    btn.url === mainRoute || btn.jsonFile === mainRoute
   );
   
   if (mainButton) {
    NavigationManager.state.isNavigating = true;
    ...
    try {
     ...
     if (mainButton.subButtons && mainButton.subButtons.length > 0) {
      // รอให้ DOM อัพเดตก่อนจัดการปุ่มย่อย
      await new Promise(resolve => setTimeout(resolve, 50));
      
      // โหลดและแสดงปุ่มย่อยทั้งหมด
      await ButtonManager.renderSubButtons(
       mainButton.subButtons,
       mainRoute,
       localStorage.getItem('selectedLang') || 'en'
      ...
      // จัดการ subRoute ถ้ามี
      if (subRoute) {
       const subButton = document.querySelector(`button[data-url="${currentHash}"]`);
       if (subButton) {
        // ทำให้ปุ่มย่อยเป็น active
        document.querySelectorAll('.sub-button').forEach(btn =>
         btn.classList.remove('active')
        );
        subButton.classList.add('active');
        
        // โหลดข้อมูลของปุ่มย่อย
       ...
       // ถ้าไม่มี subRoute ให้เลือกปุ่มย่อยที่เป็น default
       const defaultSubButton = mainButton.subButtons.find(btn => btn.isDefault);
       if (defaultSubButton) {
        const defaultFullUrl = `${mainRoute}-${defaultSubButton.url || defaultSubButton.jsonFile}`;
        const defaultButton = document.querySelector(`button[data-url="${defaultFullUrl}"]`);
        if (defaultButton) {
         defaultButton.click();
        }
       }
     ...
     await NavigationManager.updateButtonStates(currentHash);
     
    } finally {
     NavigationManager.state.isNavigating = false;
    }
   }
   
  } catch (error) {
   console.error('เกิดข้อผิดพลาดในการจัดการ popstate:', error);
   utils.showNotification(
    'เกิดข้อผิดพลาดในการนำทางย้อนกลับ กรุณาลองใหม่',
    'error'
   );
   NavigationManager.state.isNavigating = false;
  }
