 ...
 * Last updated: 2025-04-25 12:15:14
 * Author: Jeffy2600III
 */
const ScrollManager = {
    init() {
        ...
        const state = {
            ...
            subNavHeight: 0,
            isSubNavFixed: false,
            updateTimestamp: '2025-04-25 12:15:14',
        ...
        const CONSTANTS = {
            SCROLL_THRESHOLD: 5,
            DEBOUNCE_DELAY: 10,
            SUB_NAV_TOP_SPACING: 0, // Changed to 0
        ...
        /**
         * สร้าง style sheet สำหรับ sticky elements และ animations
         */
    ...
    #sub-nav {
     position: sticky;
     top: ${CONSTANTS.SUB_NAV_TOP_SPACING}px; /* Now set to 0px */
     left: 0;
     right: 0;
     z-index: ${CONSTANTS.Z_INDEX.SUB_NAV};
     transition: all ${CONSTANTS.ANIMATION_DURATION}ms ease;
   ...
   `;
        ...
        const handleSubNav = () => {
            ...
            // อัพเดทค่าตำแหน่งถ้ายังไม่มี
            if (!state.subNavOffsetTop) {
                state.subNavOffsetTop = subNav.offsetTop;
                state.subNavHeight = subNav.offsetHeight;
            }
            
            const currentScroll = window.pageYOffset;
            const triggerPoint = state.subNavOffsetTop - CONSTANTS.SUB_NAV_TOP_SPACING;
            
            if (currentScroll >= triggerPoint && !state.isSubNavFixed) {
                requestAnimationFrame(() => {
                    subNav.classList.add('fixed');
                    state.isSubNavFixed = true;
                    ...
                    Array.from(hiElements).forEach(el => {
                        el.style.transition = `all ${CONSTANTS.ANIMATION_DURATION}ms ease`;
                    });
                    ...
                    if (subButtonsContainer) {
                        subButtonsContainer.style.transition = `all ${CONSTANTS.ANIMATION_DURATION}ms ease`;
                    }
                });
            } else if (currentScroll < triggerPoint && state.isSubNavFixed) {
                requestAnimationFrame(() => {
                    subNav.classList.remove('fixed');
                    state.isSubNavFixed = false;
        ...
        // Resize Observer สำหรับอัพเดทค่าความสูง
        const resizeObserver = new ResizeObserver(entries => {
            entries.forEach(entry => {
                if (entry.target.id === 'sub-nav') {
                    state.subNavHeight = entry.contentRect.height;
                    state.subNavOffsetTop = entry.target.offsetTop;
                }
            });
        });
        ...
        // จัดการ scroll ครั้งแรก
        onScroll();
        
        // Mutation Observer สำหรับตรวจสอบการเปลี่ยนแปลง DOM
        const mutationObserver = new MutationObserver(() => {
            const subNav = document.getElementById('sub-nav');
            
            if (subNav && !state.subNavOffsetTop) {
                state.subNavOffsetTop = subNav.offsetTop;
                state.subNavHeight = subNav.offsetHeight;
            }
        });
    ...
    reset() {
        ...
        // ลบ classes และ reset styles
        const subNav = document.getElementById('sub-nav');
        
        if (subNav) {
            subNav.classList.remove('fixed');
            
            // Reset styles ของ elements ภายใน
            const subButtonsContainer = document.getElementById('sub-buttons-container');
            const hiElements = subNav.getElementsByClassName('hi');
            const hjElements = subNav.getElementsByClassName('hj');
