...
// Ultra-Optimized ContentManager: Cooperative, Prioritized, Adaptive, and Resource-Aware Rendering
// เป้าหมาย: เบาที่สุด, ไม่แย่ง resource, render ลื่น, smart adaptive, function เดิมครบถ้วน
...
window._headerV2_contentManager = {
    ...
    // Adaptive, cooperative rendering loop
    async renderContent(data) {
        ...
        const MAX_IN_DOM = 18; // Lower for memory saving
        const BATCH_SIZE_MIN = 1, BATCH_SIZE_MAX = 6; // Adaptive range
        let renderedCount = 0;
        let virtualNodes = [];
        let items = data;
        ...
        // Priority: render visible area first (viewport), then lazy for offscreen nodes
        const getVisibleBatch = () => {
            // If container is empty or user is at top, prioritize first items
        ...
        // Use browser scheduling APIs: idle, main thread, intersection observer (for lazy load)
        const progressiveRender = async (idleDeadline) => {
            if (this._isUnmounted) return;
            // Adaptive batch size based on resource availability
            let batchSize = BATCH_SIZE_MIN;
            let remain = 16;
            if (idleDeadline && idleDeadline.timeRemaining) {
                remain = idleDeadline.timeRemaining();
                batchSize = remain > 32 ? BATCH_SIZE_MAX : remain > 18 ? 3 : BATCH_SIZE_MIN;
            }
            // If page is loading or resource intensive, further reduce batchSize (can hook into global state)
            if (window._headerV2_contentLoadingManager && window._headerV2_contentLoadingManager.spinnerElement) {
            ...
            while (renderedCount < items.length && batch < batchSize) {
                const i = getVisibleBatch();
                if (renderedSet.has(i)) { renderedCount++; continue; }
                const item = items[renderedCount];
                const element = document.createElement('div');
                element.id = item.id || `content-item-${renderedCount}`;
                const innerContainer = this.createContainer(item);

                // Minimal await, only for necessary async fetches
            ...
            // Remove offscreen nodes if over max
            ...
            if (renderedCount < items.length) {
                if ('requestIdleCallback' in window) {
                    this._scheduleId = window.requestIdleCallback(progressiveRender, { timeout: 8 });
                } else {
                    this._scheduleId = setTimeout(progressiveRender, 8);
                }
            } else {
                window._headerV2_contentLoadingManager.hide();
            }
        ...
        // Start progressive loop (cooperative with browser, low priority)
        if ('requestIdleCallback' in window) {
            this._scheduleId = window.requestIdleCallback(progressiveRender, { timeout: 8 });
        } else {
            this._scheduleId = setTimeout(progressiveRender, 8);
        ...
        // Optionally use IntersectionObserver for lazy loading if data is huge (could add for images/cards)
        this._scrollListener = () => {};
        window.addEventListener('scroll', this._scrollListener);

        setTimeout(() => {
            if (container.scrollHeight <= window.innerHeight) this._scrollListener();
        }, 0);
    },

    // ฟังก์ชัน DOM/Render เดิม (ไม่ตัดอะไรเลย)
