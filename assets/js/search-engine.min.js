...
(() => {
  ...
  // ---------- localization ----------
  const LANG_TEXTS = {
    th: { all_types: "ทุกประเภท", all_categories: "ทุกหมวดหมู่", not_found: "ไม่พบข้อมูลที่ตรงหรือใกล้เคียง", copy: "คัดลอก", suggestions_for_you: "คำแนะนำสำหรับคุณ", emoji_suggestions: "คำแนะนำอิโมจิ", symbol_suggestions: "คำแนะนำสัญลักษณ์", suggestion_label: "คำค้นหาที่แนะนำ", search_placeholder: "ค้นหาข้อมูล...", type: "ประเภท", category: "หมวดหมู่", search_result_here: "ผลการค้นหาจะแสดงที่นี่", predicted: "คำแนะนำ (ประมาณการ)", emoji: "Emoji" },
    en: { all_types: "All Types", all_categories: "All Categories", not_found: "No data found related to your keyword.", copy: "Copy", suggestions_for_you: "Suggestions For You", emoji_suggestions: "Emoji Suggestions", symbol_suggestions: "Symbol Suggestions", suggestion_label: "Suggested searches", search_placeholder: "Search...", type: "Type", category: "Category", search_result_here: "Search results will appear here", predicted: "Predicted", emoji: "Emoji" }
  };
      ...
      for (let l=0;l<langs.length;l++){
        ...
        for (let j=0;j<cats.length;j++){
          ...
          for (let x=0;x<items.length;x++){
            ...
            const isFuzzy = !isMatch && bestFuzz <= Math.max(2, Math.floor(q.length * 0.25));
            if (isMatch || isFuzzy) {
              const itemName = pickLang(item.name || {}, [lang]) || '';
              results.push({
                typeObj,
      ...
      querySuggestions: function(q, maxCount) {
        ...
        for (let i=0; i<matches.length && out.length < maxCount; i++) {
          ...
          for (let cidx=0; cidx<candidates.length && out.length < maxCount; cidx++) {
            ...
            seen.add(norm);
            const highlighted = highlightMatchesHtml(raw, q);
  ...
  function renderQuerySuggestions(query) {
    ...
    for (let i=0;i<suggestions.length;i++){
      ...
      html += `<div class="suggestion-item" role="option" tabindex="0" data-val="${encodeURIComponent(s.raw)}" style="padding:8px 10px;border-top:1px solid #f4f6fa;cursor:pointer;display:flex;justify-content:space-between;align-items:center;"><span>${s.highlightedHtml}</span></div>`;
    }
  ...
  function renderSuggestionItemWithRealCategory(obj, lang) {
    ...
    const text = obj.text || '-';
    const api = obj.api || '';
    return `<div class="result-item"><div class="result-content-area"><div class="result-text-area"><span class="result-text">${escapeHtml(text)}</span><span class="result-api">${escapeHtml(api)}</span></div><div class="result-meta"><span class="result-meta-label">${typeLabel}:</span><span class="result-meta-value">${escapeHtml(typeDisplay)}</span><span style="margin-left:8px" class="result-meta-label">${catLabel}:</span><span class="result-meta-value">${escapeHtml(catName)}</span></div></div></div>`;
  }
