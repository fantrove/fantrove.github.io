/*
  search-ui.min.js (v12.1 - STABILITY IMPROVEMENTS)
...
(function () {
      ...
      try {
        ...
        if (!c) {
          c = DOMService.create('div', CONFIG.DOM.suggestionContainerId, 'search-suggestions-fullscreen', {
            position: 'relative', zIndex: '10001', width: '100%', maxHeight: 'calc(100vh - 200px)', overflow: 'auto',
  ...
  function initializeSearchEngine() {
    try {
      ...
      // click outside suggestions when overlay closed
      Handlers.documentClick = function(ev) {
        try {
          const container = DOMService.get(CONFIG.DOM.suggestionContainerId);
          const input = DOMService.get(CONFIG.DOM.searchInputId);
          if (!container) return;
      ...
      DOMService.on(document, 'click', Handlers.documentClick, false);
  ...
  function destroy() {
    try {
      ...
      OverlayService.closeSearchOverlay && OverlayService.closeSearchOverlay();
