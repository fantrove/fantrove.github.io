(function () {
  ...
  const SuggestionService = {
    ensureSuggestionContainer: function() {
      ...
      if (!c) {
        // ✅ FIXED: ลบ maxHeight และ overflow: auto
        c = DOMService.create('div', CONFIG.DOM.suggestionContainerId, 'search-suggestions-fullscreen', {
          position: 'relative', 
          zIndex: '1', 
          width: '100%',
          // ❌ ลบ: maxHeight: 'calc(100vh - 200px)',
          // ❌ ลบ: overflow: 'auto',
          overflow: 'visible',  // ✅ ให้เนื้อหาไหลผ่านได้
          background: '#fff', 
    ...
    renderQuerySuggestions: function(query) {
      ...
      for (let i = 0; i < suggestions.length; i++) {
        ...
        html += `<div class="suggestion-item" role="option" tabindex="0" data-val="${StringService.encodeUrl(s.raw)}" style="padding:12px 14px;border-top:1px solid #f4f6fa;cursor:pointer;display:flex;align-items:center;transition:all 220ms cubic-bezier(.25,.8,.35,1)">
  ...
  // ========================================
  // ✅ CENTRALIZED UI COORDINATOR SERVICE - IMPROVED
  // ========================================
  const UICoordinatorService = {
    ...
    _createOverlayElements: function(wrapper) {
      ...
      // ✅ FIXED: Overlay container ให้ scroll-y: auto เพื่อให้เลื่อนได้ทั้งหมด
      ...
      if (!overlay) {
        overlay = DOMService.create('div', CONFIG.DOM.overlayContainerId, 'search-overlay search-overlay-open', {
          ...
          overflowY: 'auto',  // ✅ เพิ่ม: ให้ overlay scroll ได้
          overflowX: 'hidden',
          pointerEvents: 'auto'  // ✅ เปลี่ยน: อนุญาตให้คลิกได้
        });
      ...
      // ✅ FIXED: Input wrapper ให้ sticky อยู่บนสุด
      DOMService.addClass(wrapper, 'overlay-elevated');
      DOMService.setStyles(wrapper, {
        position: 'sticky',  // ✅ เพิ่ม
        top: '0',            // ✅ เพิ่ม
        width: '100%', 
        maxWidth: '720px', 
        margin: '0 auto',
        marginTop: '0',      // ✅ เปลี่ยน: ไม่ต้อง margin-top
        zIndex: '10001',     // ✅ ต้องสูงสุด sticky element
        pointerEvents: 'auto',
        background: '#fff',
        borderRadius: '36px',
        boxShadow: '0 2px 12px rgba(6,14,30,0.05)'
      });
