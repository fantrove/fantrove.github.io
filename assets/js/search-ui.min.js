(function () {
  ...
  const State = {
    ...
    // ✅ ศูนย์กลาง UI Mode
    uiMode: 'ready', // 'ready' | 'overlay' | 'overlay-transitioning'
  ...
  // ========================================
  // ✅ SUGGESTION SERVICE - ปิดแค่ใน exitOverlayMode
  // ========================================
  const SuggestionService = {
    ensureSuggestionContainer: function() {
      ...
      if (!c) {
        ...
        DOMService.on(c, 'mouseenter', () => {
          State.suggestionsLocked = true;
        });
        DOMService.on(c, 'mouseleave', () => {
    ...
    handleSuggestionKeydown: function(ev, container) {
      ...
      // ✅ ลบ Escape - ไม่อนุญาตให้ปิด suggestion จากตรงนี้
    },
    ...
    handleSuggestionClick: function(ev) {
      ...
      State.suggestionsLocked = false;
      // ✅ ลบ hideSuggestions ตรงนี้ - suggestion ยังคงเปิด
      SearchService.doSearch(null, false);
    },

    renderQuerySuggestions: function(query) {
      // ✅ เฉพาะเมื่ออยู่ใน overlay mode เท่านั้น
      if (State.uiMode !== 'overlay') return;
      ...
      for (let i = 0; i < suggestions.length; i++) {
        ...
        html += `<div class="suggestion-item" role="option" tabindex="0" data-val="${StringService.encodeUrl(s.raw)}" style="padding:12px 14px;border-top:1px solid #f4f6fa;cursor:pointer;display:flex;">
  ...
  const ReadyModeService = {
    ...
    renderReadyModeSuggestions: function() {
      if (State.uiMode !== 'overlay' || !State.overlayOpenedAt) return;
  ...
  // ========================================
  // ✅ CENTRALIZED UI COORDINATOR SERVICE
  // ========================================
  const UICoordinatorService = {
    // ✅ เมื่อต้องการเปิด overlay mode
    enterOverlayMode: function() {
      if (State.uiMode !== 'ready') return;
      
      State.uiMode = 'overlay-transitioning';
      ...
      this._createOverlayElements(wrapper);
    ...
    _createOverlayElements: function(wrapper) {
        ...
        DOMService.on(backdrop, 'click', (e) => {
          if (e.target === backdrop) {
            ...
            if (currentValue !== lastQ && currentValue.length > 0) {
              ...
              UICoordinatorService.exitOverlayMode();
            }
          }
        });
    ...
    // ✅ เมื่อต้องการออกจาก overlay mode - ปิด suggestion ที่นี่เท่านั้น
    exitOverlayMode: function() {
      if (State.uiMode === 'ready') return;
      ...
      // ✅ ปิด suggestions ที่นี่ - เมื่อปิด overlay เท่านั้น
  ...
  const SearchService = {
    doSearch: function(e, preventPush, options) {
      ...
      // ✅ ลบ hideSuggestions - suggestion ยังคงเปิดอยู่
      ...
      if (!q.trim()) {
        ...
        if (State.uiMode === 'overlay' && !options.keepOverlay) UICoordinatorService.exitOverlayMode();
        return;
      }
  ...
  const UIService = {
    setupAutoSearchInput: function() {
      ...
      input.oninput = () => {
        if (State.uiMode === 'overlay-transitioning') return;
      ...
      // ✅ ลบ blur handler ที่ปิด suggestions
      DOMService.on(input, 'focus', () => {
        if (State.uiMode === 'ready') UICoordinatorService.enterOverlayMode();
      });
  ...
  // ✅ ลบ document click handler ที่ปิด suggestions
  ...
  window.__searchUI = {
    ...
    getServices: () => ({
      ...
      Coordinator: UICoordinatorService,
