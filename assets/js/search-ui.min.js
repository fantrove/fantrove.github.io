/*
  search-ui.refactored.js
  - UI layer that uses window.SearchEngine (refactored engine)
  - Centralized CONFIG, services (Language, DOM, Storage, URL, Notification)
  - RenderingService, FilterService, SuggestionService, OverlayService, SearchService, UIService
  - Exposes window.__searchUI for debugging & init
  - Compatible with previous HTML structure and IDs; minimal DOM assumptions
*/
(function () {
  'use strict';
  ...
  const URLService = {
    ...
    commitState: function(state) {
      try { if (URLService.isStateEqual(state, State.lastCommittedState)) return; } catch (e) {}
      const url = this.buildUrlForState(state);
      try {
        if (State.overlayHistoryPushed) { history.replaceState(state, '', url); State.overlayHistoryPushed = false; }
        else history.pushState(state, '', url);
      } catch (e) {
        try { history.replaceState(state, '', url); } catch (ee) {}
        State.overlayHistoryPushed = false;
      }
      StorageService.addToHistory(state);
      State.lastCommittedState = { q: state.q || '', type: state.type || 'all', category: state.category || 'all' };
    },
    removeTransientEntry: function() {
      if (!State.overlayHistoryPushed) return;
      try {
        const stateToRestore = State.lastCommittedState || { q: '', type: 'all', category: 'all' };
  ...
  const SuggestionService = {
    ensureSuggestionContainer: function() {
      ...
      if (!c) {
        c = DOMService.create('div', CONFIG.DOM.suggestionContainerId, 'search-suggestions', {
          ...
          background: '#fff', border: '1px solid #e6e9ee', boxShadow: '0 8px 30px rgba(19,23,40,0.12)', borderRadius: '10px', marginTop: '8px'
        });
        overlay.appendChild(c);
        DOMService.on(c, 'keydown', (ev) => this.handleSuggestionKeydown(ev, c));
        DOMService.on(c, 'click', (ev) => this.handleSuggestionClick(ev));
      }
      return c;
    },
    ...
    createSuggestionBackdrop: function() {
      ...
      bd = DOMService.create('div', CONFIG.DOM.suggestionBackdropId, null, {
        ...
        zIndex: '10000', background: 'transparent', pointerEvents: 'auto'
      });
      DOMService.on(bd, 'pointerdown', (ev) => {
        const sugg = DOMService.get(CONFIG.DOM.suggestionContainerId);
        if (sugg?.contains(ev.target)) return;
        State.ignoreNextHideSuggestions = true;
        setTimeout(() => { State.ignoreNextHideSuggestions = false; }, CONFIG.TIMING.ignoreNextActionDelayMs);
        OverlayService.handleBackdropClickAction();
      }, { passive: true });
    ...
    handleSuggestionKeydown: function(ev, container) {
      ...
      if (ev.key === 'ArrowDown') {
        ...
        ev.preventDefault(); this.hideSuggestions();
      }
    },
    ...
    renderQuerySuggestions: function(query) {
      ...
      if (!container) return;
      if (!query?.trim()) {
        DOMService.setHTML(container, ''); container.style.display = 'none'; this.removeSuggestionBackdrop(); return;
      }
      const suggestions = window.SearchEngine.querySuggestions(query, CONFIG.RENDER.suggestionMax);
      if (!suggestions?.length) { DOMService.setHTML(container, ''); container.style.display = 'none'; this.removeSuggestionBackdrop(); return; }
      ...
      if (inputEl) {
        inputEl.onkeydown = (e) => {
          if (e.key === 'ArrowDown') { e.preventDefault(); const first = container.querySelector('.suggestion-item'); first?.focus?.(); }
  ...
  const OverlayService = {
    openSearchOverlay: function() {
      ...
      if (!backdrop) {
        backdrop = DOMService.create('div', CONFIG.DOM.overlayBackdropId, 'search-overlay-backdrop', {
          position: 'fixed', left: '0', top: '0', width: '100%', height: '100%', background: 'rgba(12,14,18,0.48)', zIndex: '9998', backdropFilter: 'blur(4px)', pointerEvents: 'auto'
        });
        DOMService.on(backdrop, 'click', () => {
          State.ignoreNextHideSuggestions = true;
          setTimeout(() => { State.ignoreNextHideSuggestions = false; }, CONFIG.TIMING.ignoreNextActionDelayMs);
          this.handleBackdropClickAction();
        });
      ...
      if (!overlay) {
        overlay = DOMService.create('div', CONFIG.DOM.overlayContainerId, 'search-overlay search-overlay-open', {
          position: 'fixed', left: '0', top: '0', width: '100%', height: '100%', zIndex: '9999', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'flex-start', padding: '24px 16px 16px', overflow: 'auto'
        });
        document.body.appendChild(overlay);
      } else DOMService.setHTML(overlay, '');
      DOMService.addClass(wrapper, 'overlay-elevated');
      DOMService.setStyles(wrapper, { width: '100%', maxWidth: '720px', marginTop: '6px', pointerEvents: 'auto' });
      ...
      try {
        history.pushState({ __overlay: true, __overlay_from: State.lastCommittedState || null }, '', window.location.href);
        State.overlayHistoryPushed = true;
      } catch (e) { State.overlayHistoryPushed = false; }
      DOMService.on(document, 'keydown', this.overlayEscHandler);
      State.overlayOpen = true;
      const q = inputEl?.value?.trim() || '';
      if (q) {
        const candidateState = { q: q, type: State.selectedType || 'all', category: State.selectedCategory || 'all' };
        const shouldPush = !URLService.isStateEqual(candidateState, State.lastCommittedState);
        setTimeout(() => { SearchService.doSearch(null, !shouldPush, { keepOverlay: true }); }, CONFIG.TIMING.renderDelayMs);
      }
    },
    ...
    handleBackdropClickAction: function() {
      const inp = DOMService.get(CONFIG.DOM.searchInputId);
      const curr = inp?.value || '';
      if (!curr?.trim()) { URLService.removeTransientEntry(); this.closeSearchOverlay(); return; }
      if (State.preOverlayState && typeof State.preOverlayState.q !== 'undefined' && curr === (State.preOverlayState.q || '')) {
        SuggestionService.hideSuggestions(); SearchService.doSearch(null, true);
    ...
    closeSearchOverlay: function() {
      if (!State.overlayOpen) return;
      URLService.removeTransientEntry();
      const wrapper = DOMService.query('.search-input-wrapper');
      if (wrapper) {
  ...
  const SearchService = {
    doSearch: function(e, preventPush, options) {
      ...
      if (!q.trim()) {
        ...
        if (!preventPush && !State.suppressHistoryPush && !URLService.isStateEqual(stateCleared, State.lastCommittedState)) URLService.commitState(stateCleared);
      ...
      const willCommit = (!preventPush && !State.suppressHistoryPush && !URLService.isStateEqual(stateObj, State.lastCommittedState));
      if (willCommit) URLService.commitState(stateObj);
  ...
  const UIService = {
    setupAutoSearchInput: function() {
      ...
      input.onkeydown = (e) => {
        if (e.key === 'Enter') {
          e.preventDefault(); SuggestionService.hideSuggestions(); SearchService.doSearch(); this.closeMobileKeyboard();
        } else if (e.key === 'ArrowDown') {
          const container = DOMService.get(CONFIG.DOM.suggestionContainerId); if (container) { const first = container.querySelector('.suggestion-item'); first?.focus?.(); }
        }
      };
      ...
      DOMService.on(input, 'focus', () => OverlayService.openSearchOverlay());
      DOMService.on(input, 'click', () => OverlayService.openSearchOverlay());
    },
    ...
    setupMobileSelectEnter: function() {
      [CONFIG.DOM.typeFilterId, CONFIG.DOM.categoryFilterId].forEach(id => {
        ...
        el.onchange = () => { if (id === CONFIG.DOM.typeFilterId) this.onTypeChange(); else this.onCategoryChange(); };
        el.onkeyup = (e) => { if (e.key === 'Enter') { if (id === CONFIG.DOM.typeFilterId) this.onTypeChange(); else this.onCategoryChange(); } };
      });
    },
  ...
  function initializeSearchEngine() {
    fetch(CONFIG.DB.path)
      ...
      .then(data => {
        State.apiData = data;
        // init engine (async-friendly)
        window.SearchEngine.init(State.apiData, { /* options: normalizeFn:..., useWorker: false */ })
          .then(() => {
            ...
            // Restore lastCommittedState from history/session
            try {
              const hs = window.history?.state;
              if (hs && typeof hs === 'object' && (hs.q !== undefined)) {
                State.lastCommittedState = { q: hs.q || '', type: hs.type || 'all', category: hs.category || 'all' };
              } else {
                const arr = StorageService.getHistory();
                if (arr?.length) {
                  const last = arr[arr.length - 1];
                  State.lastCommittedState = { q: last.q || '', type: last.type || 'all', category: last.category || 'all' };
                } else State.lastCommittedState = null;
              }
            } catch (e) { State.lastCommittedState = null; }
            ...
            if (initial?.q) {
              try {
                ...
                State.lastCommittedState = { q: initial.q || '', type: State.selectedType || 'all', category: State.selectedCategory || 'all' };
              } finally { State.suppressHistoryPush = false; }
            } else {
              try { history.replaceState({ q: '', type: 'all', category: 'all' }, '', window.location.pathname); } catch (e) {}
              State.lastCommittedState = { q: '', type: 'all', category: 'all' };
            }
          });
      })
  ...
  document.addEventListener('pointerdown', function(ev) {
    if (!State.overlayOpen) return;
    if (ev.target.closest('#' + CONFIG.DOM.overlayContainerId)) return;
    State.ignoreNextHideSuggestions = true;
    setTimeout(() => { State.ignoreNextHideSuggestions = false; }, CONFIG.TIMING.ignoreNextActionDelayMs);
    OverlayService.handleBackdropClickAction();
  }, true);
  ...
  window.__searchUI = {
    ...
    getLastCommittedState: () => State.lastCommittedState,
